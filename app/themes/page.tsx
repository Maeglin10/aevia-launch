...
import { SOUS_SEUIL } from "@/lib/templates/catalogueGate";
...
// Screen-verified exceptions to the quality gate: an id listed here stays in the
// catalogue despite scoring below the floor. The score is a priority signal, not
// a verdict (the instrument has misjudged a hand-authored gesture before), so the
// owner keeps the final say. Empty = trust the gate.
const GARDER_MALGRE_SCORE = new Set<string>([]);

const HIDDEN_IMPACT = new Set<string>([
  // Pages missing / not renderable.
  "impact-202","impact-203","impact-204","impact-205","impact-206",
  // Below the sale-quality floor (score < 40/100) — generated by
  // scripts/gate-catalogue.mjs. A theme here is not offered for purchase until
  // reworked; its page still resolves at its direct URL.
  ...Object.keys(SOUS_SEUIL).filter((id) => !GARDER_MALGRE_SCORE.has(id)),
]);
        ...
        >
          ...
          <div className="p-5 flex flex-col flex-1 relative z-10">
            <div className="flex justify-between items-start mb-3">
              ...
              {/* données réelles uniquement : les étoiles et compteurs
                  fabriqués par hachage (theme-stats) étaient de la fausse
                  preuve sociale */}
              <div className="text-[9px] font-bold text-white/40 uppercase tracking-widest">{blocksForTheme(item.id).length} sections personnalisables</div>
            </div>
            ...
            <div className="mt-auto pt-3 border-t border-white/5 flex items-center justify-between">
              <div className="text-[8px] font-bold text-white/20 uppercase tracking-widest">Aperçu personnalisé en 5 min</div>
