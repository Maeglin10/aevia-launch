      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${scrolled ? "bg-[#faf6f1]/98 backdrop-blur-xl py-3 shadow-sm border-b border-[var(--brand,#c97b7b)]/10" : "bg-transparent py-7"}`}>
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0145678901").replace(/[^+0-9]/g, "")}`} className="hidden md:flex items-center gap-2 text-[var(--brand,#c97b7b)] font-bold text-sm">
              <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "01 45 67 89 01"}
            </a>
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="bg-[#faf6f1] border-slate-100 p-10">
                <div className="flex flex-col gap-7 mt-16">
                  ...
                  <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0145678901").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 text-[var(--brand,#c97b7b)] font-bold text-xl mt-4"><Phone className="w-5 h-5" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "01 45 67 89 01"}</a>
                </div>
              </SheetContent>
            </Sheet>
          </div>
      ...
      <section id="hero" ref={heroRef} className="relative h-[100dvh] min-h-[640px] flex items-end overflow-hidden">
        ...
        <motion.div style={{ y: heroTextY, opacity: heroOpacity }} className="relative z-10 max-w-[1400px] w-full mx-auto px-6 md:px-12 pb-28">
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.0 }} className="flex flex-wrap gap-4">
            ...
            <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0145678901").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 px-9 py-4 border border-white/12 text-white/50 font-bold text-[10px] uppercase tracking-widest hover:border-[var(--brand,#c97b7b)]/40 hover:text-[#d4a5a5] transition-all">
              <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "01 45 67 89 01"}
            </a>
          </motion.div>
        </motion.div>
        ...
        <Reveal>
          <div className="max-w-xl mx-auto px-6">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              ...
              <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0145678901").replace(/[^+0-9]/g, "")}`} className="min-h-[44px] flex items-center gap-3 px-10 py-4 border border-white/12 text-white/40 font-bold text-[10px] uppercase tracking-widest hover:border-[var(--brand,#c97b7b)]/40 hover:text-[#d4a5a5] transition-all cursor-pointer">
                <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "01 45 67 89 01"}
              </a>
            </div>
          </div>
      ...
      <footer className="bg-[#110c10] pt-20 pb-10 px-6 border-t border-white/5">
        <div className="max-w-[1300px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          {[
            ...
            { t: "Nous trouver", ls: [(clientAddress(sessionData) ? "" : "38 av. Victor Hugo"), clientCodePostalVille(sessionData, "75016", "Paris"), "Mar-Sam 9h-19h", (clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "01 45 67 89 01"), (fd?.email ?? "contact@atelier-leonie.fr")] },
          ].map((col, i) => (
            <div key={i}>
        ...
        <div className="max-w-[1300px] mx-auto pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between gap-3 text-[10px] font-bold uppercase tracking-widest text-white/8">
          <span>© 2026 {clientName(sessionData) ?? "Atelier Léonie"}{clientSiret(sessionData) ? ` · SIRET ${clientSiret(sessionData)}` : clientName(sessionData) ? "" : " · SIRET 234 567 890 00011"} · {clientCity(sessionData) ?? "Paris"} (75016){/* VILLE_PIED */}{clientCity(sessionData) ? ` · ${clientCity(sessionData)}` : ""}</span>
