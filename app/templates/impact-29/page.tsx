...
return (
    <div ref={containerRef} className="relative w-full overflow-hidden">
      {/* Hero */}
      <section className="min-h-dvh flex items-center relative overflow-hidden pt-20">
        <div className="absolute inset-0">
          <div className="absolute top-1/3 left-1/2 w-[600px] h-[600px] -translate-x-1/2 -translate-y-1/2 bg-[var(--brand,#00F5D4)]/5 rounded-full blur-3xl" />
        ...
        <motion.div style={{ y: heroY }} className="max-w-6xl mx-auto px-6 relative w-full">
          ...
          >
            <div><span className="text-[var(--brand,#00F5D4)]">const </span>role = <span className="text-emerald-400">"Staff Engineer & Open Source"</span></div>
            <div><span className="text-[var(--brand,#00F5D4)]">const </span>location = <span className="text-emerald-400">"Paris, France"</span></div>
            <div><span className="text-[var(--brand,#00F5D4)]">const </span>available = <span className="text-emerald-400">"Q3 2026 contracts"</span></div>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <Link href="/templates/impact-29/work" className="bg-[var(--brand,#00F5D4)] text-[#0A0E1A] font-bold text-sm px-6 py-3 hover:bg-[#00E5C4] transition-colors flex items-center gap-2 cursor-pointer border border-[var(--brand,#00F5D4)]">
              view work() <ArrowRight className="w-4 h-4" />
            </Link>
            <a href="https://github.com" target="_blank" rel="noopener noreferrer" className="border border-[var(--brand,#00F5D4)]/30 text-[var(--brand,#00F5D4)] text-sm px-6 py-3 hover:bg-[var(--brand,#00F5D4)]/10 transition-colors flex items-center gap-2 cursor-pointer">
          ...
          <motion.div
            ...
            className="bg-[#0D1323] border border-[var(--brand,#00F5D4)]/20 rounded-lg overflow-hidden max-w-2xl"
          >
            <div className="flex items-center gap-2 px-4 py-3 bg-[#0F1729] border-b border-[var(--brand,#00F5D4)]/10">
            ...
            <div className="p-4 text-sm space-y-2">
              {[
                { p: "$ ", t: "git clone https://github.com/glitchdev/vaultkey", c: "text-[var(--brand,#00F5D4)]" },
                { p: "✓ ", t: "Cloned in 0.8s", c: "text-emerald-400" },
                { p: "$ ", t: "cd vaultkey && cargo build --release", c: "text-[var(--brand,#00F5D4)]" },
      ...
      <section className="py-24 px-6 border-t border-[var(--brand,#00F5D4)]/10">
            ...
            {projects.map((p, i) => (
              <button
                ...
                className={`px-4 py-2 text-sm font-semibold transition-all cursor-pointer border ${
                  activeProject === i
                    ? "bg-[var(--brand,#00F5D4)] text-[#0A0E1A] border-[var(--brand,#00F5D4)]"
                    : "border-[var(--brand,#00F5D4)]/20 text-[#475569] hover:border-[var(--brand,#00F5D4)]/50 hover:text-[#E2E8F0]"
                }`}
              >
                {p.name}
              </button>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <ScrollImage
                ...
                width={800}
                height={500}
                className="w-full aspect-video border border-[var(--brand,#00F5D4)]/20"
              ...
              <div>
                ...
                <div className="flex gap-4">
                  <a href="https://github.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-[var(--brand,#00F5D4)] hover:opacity-70 transition-opacity cursor-pointer">
      ...
      {/* Tech Stack */}
      <section className="py-24 px-6 bg-[#0D1323] border-t border-b border-[var(--brand,#00F5D4)]/10">
        <div className="max-w-6xl mx-auto">
          <Reveal className="mb-12">
            <div className="flex items-end justify-between">
              <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {skills.map((s, i) => (
              <Reveal key={s.cat} delay={i * 0.1}>
                <div className="border border-[var(--brand,#00F5D4)]/10 p-6 hover:border-[var(--brand,#00F5D4)]/30 transition-colors">
                  <div className="text-[var(--brand,#00F5D4)] text-xs mb-4 uppercase tracking-widest">{s.cat}</div>
                  <ul className="space-y-2">
                    {s.items.map(item => (
                      <li key={item} className="flex items-center gap-2 text-sm text-[#94A3B8]">
                        <ChevronRight className="w-3 h-3 text-[var(--brand,#00F5D4)] shrink-0" />
                        {item}
                      </li>
                    ))}
                  </ul>
      ...
      {/* Experience Section */}
      <section className="py-24 px-6 bg-[#0D1323] border-t border-[var(--brand,#00F5D4)]/10">
        <div className="max-w-6xl mx-auto">
          <Reveal className="mb-12">
            <div className="flex items-end justify-between">
              <div>
          ...
          <div className="relative border-l border-[var(--brand,#00F5D4)]/20 pl-8 space-y-10">
            {timeline.slice(0, 3).map((t, i) => (
              <Reveal key={t.year} delay={i * 0.1}>
                <div className="relative">
                  <div className="absolute -left-[2.6rem] w-3 h-3 border-2 border-[var(--brand,#00F5D4)] bg-[#0D1323] rounded-full" />
                  <div className="text-[var(--brand,#00F5D4)] text-xs mb-2">{t.year}</div>
          ...
          >
            {[...stats, ...stats, ...stats].map((s, i) => (
              <div key={i} className="flex items-center gap-6 shrink-0">
                <div className="border border-[var(--brand,#00F5D4)]/20 bg-[#0D1323] px-6 py-4 min-w-[200px]">
                  <div className="text-[var(--brand,#00F5D4)] text-2xl font-bold">{s.value}</div>
                  <div className="text-[#94A3B8] text-xs mt-1">{s.label}</div>
                  <div className="text-[#475569] text-xs mt-0.5">{s.detail}</div>
                </div>
                <div className="text-[var(--brand,#00F5D4)]/20 text-lg select-none">{"///"}</div>
              </div>
            ))}
          </motion.div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {services.map((svc, i) => (
              <Reveal key={svc.id} delay={i * 0.08}>
                <div className="border border-[var(--brand,#00F5D4)]/15 p-8 hover:border-[var(--brand,#00F5D4)]/40 transition-all group cursor-default h-full flex flex-col">
                  <div className="flex items-start justify-between mb-6">
                    <div className="text-[#475569] text-xs font-bold">{svc.id}</div>
                    <div className="text-[var(--brand,#00F5D4)] text-xs font-bold border border-[var(--brand,#00F5D4)]/30 px-3 py-1">{svc.rate}</div>
                  </div>
                  <h3 className="text-xl font-bold mb-2 group-hover:text-[var(--brand,#00F5D4)] transition-colors">{svc.name}</h3>
                  <div className="text-[var(--brand,#00F5D4)] text-xs mb-4">{svc.short}</div>
                  <p className="text-[#94A3B8] text-sm leading-relaxed mb-6 flex-1">{svc.body}</p>
                  <div className="flex flex-wrap gap-2">
                    {svc.tags.map(tag => (
                      <span key={tag} className="text-xs border border-[var(--brand,#00F5D4)]/20 text-[#475569] px-2.5 py-1">{tag}</span>
                    ))}
                  </div>
                </div>
              </Reveal>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0">
            {process.map((step, i) => (
              <Reveal key={step.step} delay={i * 0.1}>
                <div className="border border-[var(--brand,#00F5D4)]/15 p-8 border-r-0 last:border-r border-b md:border-b-0 relative hover:bg-[#0D1323] transition-colors">
                  {/* Connector dot */}
                  {i < process.length - 1 && (
                    <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 w-2 h-2 bg-[var(--brand,#00F5D4)] z-10 hidden lg:block" />
                  )}
                  <div className="text-[var(--brand,#00F5D4)] font-bold text-xs mb-6">{step.step}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {testimonials.map((t, i) => (
              <Reveal key={t.name} delay={i * 0.08}>
                <div className="border border-[var(--brand,#00F5D4)]/15 p-8 hover:border-[var(--brand,#00F5D4)]/30 transition-colors h-full flex flex-col">
                  <div className="text-[var(--brand,#00F5D4)] text-lg mb-6 select-none">&ldquo;</div>
                  <blockquote className="text-[#E2E8F0] text-sm leading-relaxed flex-1 mb-8">{t.quote}</blockquote>
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full overflow-hidden border border-[var(--brand,#00F5D4)]/20 shrink-0">
      ...
      {/* ── CLIENTS LOGOS ── */}
      <section className="py-20 px-6 border-t border-[var(--brand,#00F5D4)]/10">
        <div className="max-w-6xl mx-auto">
          <Reveal className="mb-12 text-center">
            <div className="text-[var(--brand,#00F5D4)] text-xs mb-2"><span className="text-[#475569]">// </span>previous_employers_and_clients</div>
            <p className="text-[#475569] text-sm">Companies I've shipped production code for</p>
          </Reveal>
          <div className="grid grid-cols-4 md:grid-cols-8 gap-px bg-[var(--brand,#00F5D4)]/10">
            {clients.map((c, i) => (
              <Reveal key={c.name} delay={i * 0.04}>
                <div className="bg-[#0A0E1A] flex items-center justify-center py-8 px-4 hover:bg-[#0D1323] transition-colors group cursor-default">
                  <span className="text-[#475569] text-xs font-bold tracking-widest group-hover:text-[var(--brand,#00F5D4)] transition-colors">{c.logo}</span>
                </div>
              </Reveal>
            ))}
          </div>
              ...
              <Reveal key={p.name} delay={i * 0.1}>
                <Link href="/templates/impact-29/work" className="group block border border-[var(--brand,#00F5D4)]/15 overflow-hidden hover:border-[var(--brand,#00F5D4)]/40 transition-all cursor-pointer">
                  <div className="relative aspect-video overflow-hidden">
                    ...
                    <div className="absolute top-3 right-3 text-xs border border-[var(--brand,#00F5D4)]/40 text-[var(--brand,#00F5D4)] px-2 py-1 bg-[#0A0E1A]/80">{p.year}</div>
                  ...
                  <div className="p-5">
                    ...
                    <div className="font-bold text-[#E2E8F0] group-hover:text-[var(--brand,#00F5D4)] transition-colors flex items-center gap-2">
                      {p.name}
      ...
      {/* ── CTA BANNER ── */}
      <section className="py-24 px-6 border-t border-[var(--brand,#00F5D4)]/10 relative overflow-hidden">
        <div className="absolute inset-0 opacity-[0.04]" style={{
          backgroundImage: `linear-gradient(rgba(0,245,212,1) 1px, transparent 1px), linear-gradient(90deg, rgba(0,245,212,1) 1px, transparent 1px)`,
          backgroundSize: "60px 60px"
        }} />
        <div className="absolute top-1/2 left-1/2 w-[800px] h-[400px] -translate-x-1/2 -translate-y-1/2 bg-[var(--brand,#00F5D4)]/5 rounded-full blur-3xl pointer-events-none" />
        <div className="max-w-4xl mx-auto text-center relative">
          <Reveal>
            <div className="text-[var(--brand,#00F5D4)] text-xs mb-4"><span className="text-[#475569]">// </span>next_step</div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link
                ...
                className="bg-[var(--brand,#00F5D4)] text-[#0A0E1A] font-bold text-sm px-8 py-4 hover:bg-[#00E5C4] transition-colors flex items-center gap-2 cursor-pointer w-full sm:w-auto justify-center"
              >
                Start a conversation <ArrowRight className="w-4 h-4" />
              </Link>
              <Link
                href="/templates/impact-29/work"
                className="border border-[var(--brand,#00F5D4)]/30 text-[var(--brand,#00F5D4)] text-sm px-8 py-4 hover:bg-[var(--brand,#00F5D4)]/10 transition-colors flex items-center gap-2 cursor-pointer w-full sm:w-auto justify-center"
              >
                <Code2 className="w-4 h-4" /> Browse the work
              </Link>
            </div>
            <div className="mt-10 text-xs text-[#475569]">
              <span className="text-[var(--brand,#00F5D4)]">const </span>availability = <span className="text-emerald-400">"Q3 2026 — 2 slots remaining"</span>
            </div>
          </Reveal>
        </div>
      </section>
      ...
      {/* Contact Form */}
      <section id="contact" className="py-32 px-6 border-t border-[var(--brand,#00F5D4)]/10">
        <div className="max-w-3xl mx-auto text-center">
          <Reveal>
            ...
            <div className="space-y-4">
              <a href={`mailto:${fd?.email ?? "hello@aevia.ws"}`} className="block w-full bg-[var(--brand,#00F5D4)] text-[#0A0E1A] font-bold text-sm py-4 hover:bg-[#00E5C4] transition-colors cursor-pointer text-center">
                hello@aevia.ws →
              </a>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Link href="/templates/impact-29/timeline" className="border border-[var(--brand,#00F5D4)]/20 text-[var(--brand,#00F5D4)] text-sm py-4 hover:bg-[var(--brand,#00F5D4)]/10 transition-colors cursor-pointer flex items-center justify-center gap-2">
