      ...
      >
        ...
        >
          {fd?.logoBase64 ? (
            ...
            <>
          {clientName(__layoutSession) ? null : "GHOST"}<span style={{ color: C.white }}>{/* NOM_LOGO */ clientName(__layoutSession) ?? "PROTOCOL"}</span>
        </>
          )}</Link>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 48, marginBottom: 64 }}>
            <div>
              <div style={{ fontSize: 22, fontWeight: 900, color: C.green, letterSpacing: "0.12em", marginBottom: 16, textShadow: `0 0 15px ${C.green}` }}>
                {clientName(__layoutSession) ? null : "GHOST"}<span style={{ color: C.white }}>{clientName(__layoutSession) ?? "PROTOCOL"}</span>
              </div>
