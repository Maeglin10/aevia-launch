...
import { ArrowRight, CheckCircle, Clock, Leaf, Mail, MapPin, Phone, Shirt, Sparkles, Star } from "lucide-react";
...
/* Pressing / blanchisserie, 1re variante. Signature : WipeReveal — le vêtement propre révélé d'un passage, littéralement le geste du métier. Carte CSS, sans photographie. */
...
const NAV = [{"l": "Soins", "h": "#services"}, {"l": "Comment ça marche", "h": "#methode"}, {"l": "Tarifs", "h": "#tarifs"}, {"l": "Contact", "h": "#contact"}];
const HERO = [{"k": "Costumes & tailleurs", "line": "Tombé impeccable, épaules vivantes.", "sub": "Aquanettoyage + apprêt vapeur main."}, {"k": "Robes & soie", "line": "Les matières délicates, traitées à la main.", "sub": "Soie, cachemire, dentelle — fiche de soin par pièce."}, {"k": "Linge de maison", "line": "Draps et nappes rendus comme à l'hôtel.", "sub": "Calandrage professionnel, pliage boutique."}];

const SERVICES_SOURCE = [{"titre": "Aquanettoyage", "desc": "Nettoyage à l'eau haute précision : programmes par fibre, détergents biodégradables, zéro solvant chloré. Meilleur pour les fibres, pour vous et pour l'atelier.", "tag": "Écologique"}, {"titre": "Costumes & manteaux", "desc": "Détachage main, nettoyage, apprêt vapeur sur mannequin : les épaules gardent leur forme, les revers leur roulé.", "tag": "Homme"}, {"titre": "Robes, soie & delicates", "desc": "Soie, mousseline, perles et sequins : traitement à la main, séchage à plat, fiche de soin pièce par pièce.", "tag": "Délicat"}, {"titre": "Robes de mariée", "desc": "Nettoyage après le grand jour et conservation en boîte anti-acide avec papier de soie — prête pour trente ans de souvenirs.", "tag": "Mariage"}, {"titre": "Linge de maison", "desc": "Couettes, draps, nappes damassées : lavage grand volume, calandrage, pliage boutique. Retrait sous 72 h.", "tag": "Maison"}, {"titre": "Retouches & couture", "desc": "Ourlets, fermetures, boutons, cintrage de veste : l'atelier couture répare pendant que le pressing nettoie.", "tag": "Retouches"}];
...
export default function BlancNetPage() {
  ...
  const phone = fd?.phone ?? "01 45 00 00 00";
  const telHref = `tel:${fd?.phone ?? "+33145000000"}`;
  const mail = fd?.email ?? "atelier@blanc-net.fr";

  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: FONT_BODY, overflowX: "clip" }}>
      <style>{`
        @media (max-width: 900px) { #i343-nav { display: none !important; } .i343-burger { display: flex !important; } }
        @media (max-width: 860px) {
          .i343-hero { grid-template-columns: 1fr !important; padding: 118px 24px 46px !important; gap: 34px !important; }
          .i343-card { max-width: 380px; margin: 0 auto; width: 100%; }
          .i343-split { grid-template-columns: 1fr !important; }
          .i343-stats { grid-template-columns: 1fr 1fr !important; row-gap: 8px; }
          .i343-stats .i343-statcell { border-right: none !important; }
          .i343-pad { padding-left: 24px !important; padding-right: 24px !important; }
          .i343-herotext { padding: 0 24px 44px !important; }
        }
      `}</style>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, height: 72, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 48px", background: scrolled ? C.bg : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", borderBottom: `1px solid ${scrolled ? C.border : "transparent"}`, transition: "all 0.4s ease" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          {fd?.logoBase64 ? (
            ...
            <>
              <Shirt size={18} color={C.accent} style={{ flexShrink: 0 }} />
              <span style={{ fontFamily: FONT, fontSize: 18, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{fd?.businessName ?? "Blanc Net"}</span>
              <span style={{ fontSize: 10, letterSpacing: 2.2, textTransform: "uppercase", color: C.textMuted, marginLeft: 6 }}>Pressing</span>
            </>
          )}
        </div>
        <div id="i343-nav" style={{ display: "flex", gap: 24, alignItems: "center" }}>
          {NAV.map(({ l, h }) => (
        ...
        <button className="i343-burger" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Menu" style={{ display: "none", flexDirection: "column", justifyContent: "center", gap: 5, background: "none", border: "none", cursor: "pointer", padding: 10, minWidth: 44, minHeight: 44 }}>
      ...
      {mobileOpen && (
        <div style={{ position: "fixed", top: 72, left: 0, right: 0, zIndex: 99, background: C.bg, borderBottom: `1px solid ${C.border}`, padding: "20px 28px", display: "flex", flexDirection: "column", gap: 4 }}>
          ...
          <a href="#contact" style={{ background: C.accentDark, color: "#fff", borderRadius: 8, padding: "13px 22px", fontSize: 15, fontWeight: 700, textDecoration: "none", textAlign: "center", marginTop: 8 }}>Nous trouver</a>
        </div>
      )}

      {/* ── HERO ────────────────────────────────────────────────────────── */}
<section className="i343-hero" style={{ minHeight: "100dvh", display: "grid", gridTemplateColumns: "minmax(0,1.08fr) minmax(0,0.92fr)", gap: 56, alignItems: "center", padding: "140px 64px 70px", maxWidth: 1260, margin: "0 auto" }}>
        <div>
          <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} style={{ fontSize: 12, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", color: C.accentDark }}>
            Pressing · {clientCity(sessionData) ?? "Paris"}
          </motion.span>
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.85, ease: [0.16, 1, 0.3, 1] }} style={{ fontFamily: FONT, fontSize: "clamp(34px, 4.6vw, 60px)", color: C.text, lineHeight: 1.1, margin: "18px 0 20px" }}>{/* TEXTE_SECTION */ clientText(sessionData, "section-1.titre") ?? (<>
            {c?.heroHeadline ?? (<>{clientHeroLine(sessionData, 0, 2, 20) ?? "Vos vêtements rendus"}<br /><em style={{ color: C.accentDark }}>{clientHeroLine(sessionData, 1, 2, 20) ?? "mieux que confiés."}</em></>)}
          </>)}</motion.h1>
        ...
        <div className="i343-card">
          <WipeReveal index={i}>
            <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden", boxShadow: "0 18px 52px rgba(0,0,0,0.18)" }}>
              <div style={{ aspectRatio: "4/3", background: C.accentLight , overflow: "hidden" }}><img src={photo(0, (clientPhotos(sessionData)[0] || "https://images.pexels.com/photos/8774451/pexels-photo-8774451.jpeg?auto=compress&cs=tinysrgb&w=1400"))} alt="Atelier de pressing en activité" loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /></div>
      ...
      <section style={{ background: C.bgDark }}>
        <div className="i343-stats i343-pad" style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", maxWidth: 1100, margin: "0 auto", padding: "0 32px" }}>
          {STATS.map((s, idx) => (
            <Reveal key={s.label} delay={idx * 0.08}>
              <div className="i343-statcell" style={{ padding: "30px 8px", textAlign: "center", borderRight: idx < 3 ? "1px solid rgba(255,255,255,0.08)" : "none" }}>
      ...
      {/* ── SERVICES ────────────────────────────────────────────────────── */}
      <section id="services" className="i343-pad" style={{ padding: "96px 64px", background: C.bgSection }}>
          ...
          <Reveal>
            <div style={{ marginBottom: 50 }}>
              ...
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 3.8vw, 46px)", color: C.text, marginTop: 10, lineHeight: 1.14 }}>{/* TEXTE_SECTION */ clientText(sessionData, "services.titre") ?? (<>
                Chaque matière<br /><em>a son protocole.</em>
              </>)}</h2>
            </div>
          </Reveal>
      ...
      {/* ── MÉTHODE / INFOS ─────────────────────────────────────────────── */}
      <section id="methode" className="i343-pad" style={{ padding: "96px 64px", background: C.bg }}>
          ...
          <Reveal>
            <div style={{ marginBottom: 50 }}>
              ...
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 3.8vw, 46px)", color: C.text, marginTop: 10, lineHeight: 1.14 }}>{/* TEXTE_SECTION */ clientText(sessionData, "methode.titre") ?? (<>
                Déposez le matin,<br /><em>oubliez le reste.</em>
              </>)}</h2>
            </div>
          </Reveal>
      ...
      {/* ── ENGAGEMENTS ─────────────────────────────────────────────────── */}
      <section id="engagements" className="i343-pad" style={{ padding: "96px 64px", background: C.bgSection }}>
        <div className="i343-split" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 64, alignItems: "center" }}>
          <Reveal>
            <div style={{ borderRadius: 12, border: `1px solid ${C.border}`, background: C.accentLight, aspectRatio: "4/3", display: "flex", alignItems: "center", justifyContent: "center" }}><Leaf size={80} color={C.accentDark} strokeWidth={1.1} /></div>
          </Reveal>
          <Reveal delay={0.15}>
            <div>
              ...
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(26px, 3vw, 40px)", color: C.text, margin: "12px 0 26px", lineHeight: 1.18 }}>{/* TEXTE_SECTION */ clientText(sessionData, "engagements.titre") ?? (<>
                Propre,<br /><em>dans tous les sens.</em>
              </>)}</h2>
      ...
      {/* ── TARIFS ──────────────────────────────────────────────────────── */}
      <section id="tarifs" className="i343-pad" style={{ padding: "96px 64px", background: C.bg }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              ...
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 3.5vw, 44px)", color: C.text, marginTop: 10 }}>{/* TEXTE_SECTION */ clientText(sessionData, "tarifs.titre") ?? (<>Affichés, <em>tout compris.</em></>)}</h2>
              <p style={{ fontSize: 15, color: C.textMuted, maxWidth: 560, margin: "14px auto 0", lineHeight: 1.7 }}>Détachage courant et apprêt compris dans le prix. Les pièces très tachées font l'objet d'un devis au comptoir, avant traitement.</p>
            </div>
          </Reveal>
      ...
      {/* ── AVIS ────────────────────────────────────────────────────────── */}
      <section className="i343-pad" style={{ padding: "96px 64px", background: C.bgDark }}>
        <Reveal>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <h2 style={{ fontFamily: FONT, fontSize: "clamp(26px, 3.4vw, 42px)", color: "#fff" }}>{/* TEXTE_SECTION */ clientText(sessionData, "section-7.titre") ?? (<>Ils reviennent <em style={{ color: C.hi }}>chaque semaine</em>.</>)}</h2>
          </div>
        </Reveal>
      ...
      {/* ── CONTACT ─────────────────────────────────────────────────────── */}
      <section id="contact" className="i343-pad" style={{ padding: "96px 64px", background: C.accentLight, textAlign: "center" }}>
        <Reveal>
          <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", color: C.accentDark }}>Rue du Commerce</span>
          <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 4vw, 48px)", color: C.text, margin: "14px 0 16px" }}>{/* TEXTE_SECTION */ clientText(sessionData, "contact.titre") ?? (<>
            Déposez en passant,<br /><em>on s'occupe du reste.</em>
          </>)}</h2>
          <p style={{ fontSize: 16, color: C.textMuted, maxWidth: 460, margin: "0 auto 36px", lineHeight: 1.7 }}>Sans rendez-vous aux heures d'ouverture. Dépôt avant 10 h = prêt le lendemain 17 h.</p>
      ...
      {/* ── FOOTER ──────────────────────────────────────────────────────── */}
      <footer className="i343-pad" style={{ background: C.bgDark, padding: "44px 64px 22px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 28, marginBottom: 30 }}>
            <div>
              <div style={{ fontFamily: FONT, fontSize: 18, color: C.hi, marginBottom: 8 }}>{fd?.businessName ?? "Blanc Net"}</div>
              <p style={{ color: "rgba(255,255,255,0.38)", fontSize: 13, lineHeight: 1.7 }}>Pressing & blanchisserie · {clientCity(sessionData) ?? "Paris"}<br />Aquanettoyage — zéro perchloroéthylène depuis 2020</p>
            </div>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.09)", paddingTop: 14, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
            <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 12 }}>
              © 2026 {fd?.businessName ?? "Blanc Net"} — Site réalisé par Aevia WS · SIREN {/* VILLE_PIED */}{clientCity(sessionData) ? ` · ${clientCity(sessionData)}` : ""}<LegalIdentity />
            </span>
