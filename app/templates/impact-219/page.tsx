...
let C: Record<string, string> = {
  ...
  border: '#e8e6f2',
  borderStrong: '#d8d4ec',
  primary: 'var(--brand,#6d4aff)',
  primaryDark: 'var(--brand-light,#5a37e0)',
  primaryLight: '#8b6dff',
  indigo: '#4338ca',
  ink: '#15131f',
  inkSoft: '#4a4660',
    ...
    >
      ...
      >
        ...
        >
          ...
          <p style={{ fontSize: 18, color: C.inkSoft, lineHeight: 1.65, maxWidth: 480, margin: '0 0 32px' }}>{clientHeroPrestations(sessionData) ?? "NovaSaaS réunit vos données, vos équipes et vos automatisations dans une seule plateforme. Décidez plus vite, avec des analyses en temps réel."}</p>
...
const AV = (id: string) =>
  ((id).startsWith('http') ? (id) : (clientPhotos(sessionData)[0] || `https://images.unsplash.com/photo-${id}?q=80&w=160&h=160&auto=format&fit=crop&crop=faces`));
...
const TESTIS_SOURCE: Testi[] = [
  ...
  { name: 'Karl Nyström', role: 'Data Lead, Nordata', hue: 'var(--brand-light,#5a37e0)', img: AV('1507003211169-0a1dd7228f2d'), body: 'Enfin un outil que mon équipe data ET mon équipe marketing utilisent sans se battre.' },
