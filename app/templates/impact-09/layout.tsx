...
export default function impact09Layout({
  children,
}: {
  ...
  useEffect(() => {
    ...
    try {
      const cle = "apercu-session:impact-09";
  ...
  return (
    <>
      {surUneAnnexe && (
        <EnteteAnnexe session={session} repli="ASTRUM REACH ORBITAL" accueil="/templates/impact-09" />
      )}
      {children}
    </>
  );
