...
function PHOTO_LIVE() {
  ...
  mannequin:
    (clientPhotos(sessionData)[2] || 'https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?q=80&w=1600&auto=format&fit=crop'),
  tissu:
    ...
    <section id="hero" ref={ref} style={section}>
      ...
      <motion.div
        style={{
          ...
          padding: 'clamp(96px,14vh,132px) clamp(24px,6vw,96px) clamp(88px,17vh,160px)',
          y: titleY,
    ...
    <section style={sec} id="materiaux">
      <div style={{ maxWidth: 1240, margin: '0 auto 64px' }}>
        ...
        <Reveal delay={0.08}>
          ...
          >{/* TEXTE_SECTION */ clientText(sessionData, "materiaux.titre") ?? (<>
            L&apos;étoffe fait{' '}
            <span style={{fontStyle: 'italic', color: brand ?? 'var(--brand,#7a5c2e)' }}>
              la tenue
            </span>
    ...
    <section style={sec} id="creations">
      <div style={{ maxWidth: 1240, margin: '0 auto 64px' }}>
        ...
        <Reveal delay={0.08}>
          ...
          >{/* TEXTE_SECTION */ clientText(sessionData, "creations.titre") ?? (<>
            L&apos;art de la{' '}
            <span style={{ fontStyle: 'italic', color: C.goldLight }}>
              silhouette
            </span>
  ...
  const cols: { title: string; items: { label: string; href: string }[] }[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { label: (clientPhone(sessionData) ?? '01 42 65 78 37'), href: `tel:${(clientPhone(sessionData) ?? '+33142650000').replace(/[^+0-9]/g, "")}` },
        { label: (clientEmail(sessionData) ?? 'contact@maissonceleste.fr'), href: `mailto:${clientEmail(sessionData) ?? 'contact@maisonceleste.fr'}` },
