      ...
      <footer style={{ background: C.navy, padding: "56px 80px 28px", fontFamily: FONT }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 32, marginBottom: 40 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[{ icon: <MapPin size={14} />, t: (clientCity({ formData: fd }) ?? "Paris") + " & agences IDF" }, { icon: <Phone size={14} />, t: (clientPhone(sessionData) ?? fd?.phone ?? "01 40 00 00 00") }, { icon: <Mail size={14} />, t: (clientEmail(sessionData) ?? fd?.email ?? "contact@pierreandco.fr") }].map((item, i) => (
