...
let C: Record<string, string> = {
  primary: "var(--brand,#3949ab)",
  primaryLight: "var(--brand-light,#5562c4)",
  primaryDark: "#283593",
  bg: "#fafbff",
  bgDeep: "#f0f2ff",
  bgCard: "#ffffff",
...
function PHOTO_LIVE() {
  return {
  hero: (clientPhotos(sessionData)[0] || "https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=1600&auto=format&fit=crop"),
  about: (clientPhotos(sessionData)[1] || "https://images.unsplash.com/photo-1552664730-d307ca884978?q=80&w=1600&auto=format&fit=crop"),
  special: (clientPhotos(sessionData)[2] || "https://images.unsplash.com/photo-1551836022-d5d88e9218df?q=80&w=1600&auto=format&fit=crop"),
  gallery1: (clientPhotos(sessionData)[3] || "https://images.unsplash.com/photo-1497215728101-856f4ea42174?q=80&w=800&auto=format&fit=crop"),
  gallery2: (clientPhotos(sessionData)[4] || "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?q=80&w=800&auto=format&fit=crop"),
  gallery3: (clientPhotos(sessionData)[5] || "https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=800&auto=format&fit=crop"),
  gallery4: (clientPhotos(sessionData)[6] || "https://images.unsplash.com/photo-1498050108023-c5249f4df085?q=80&w=800&auto=format&fit=crop")
} as const;
}
        ...
        <div style={{
          ...
          <a href="#hero" style={{
            ...
            {fd?.logoBase64 ? (
              ...
              fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "Nexus Compta"))
            )}
          </a>
      ...
      >
        ...
        <div style={{
          ...
          <Reveal delay={0.25}>
            ...
            }}>{<>{clientHeroLine(sessionData, 0, 2, 21) ?? "L'Expertise Comptable"}<br />{clientHeroLine(sessionData, 1, 2, 21) ?? "des Entrepreneurs"}</>}</h1>
          </Reveal>
      ...
      <section id="about" style={{ padding: '120px 24px', background: C.bg }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{
            ...
            <div>
              <Reveal delay={0.15}>
                <Eyebrow>{fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "Nexus Compta"))}</Eyebrow>
                <h2 style={{
                  fontFamily: SERIF,
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                  <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                    ...
                    <div>
                      ...
                      <a href={`tel:${(clientPhone(sessionData) ?? fd?.phone ?? "+33572755919").replace(/[^+0-9]/g, "")}`} style={{ fontSize: 15, color: C.text, fontWeight: 700, textDecoration: 'none' }}>{clientPhone(sessionData) ?? fd?.phone ?? "+33 (0)5 00 00 00 00"}</a>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                    ...
                    <div>
                      ...
                      <a href={`mailto:${clientEmail(sessionData) ?? fd?.email ?? "contact@mysite.com"}`} style={{ fontSize: 15, color: C.text, fontWeight: 700, textDecoration: 'none' }}>{clientEmail(sessionData) ?? fd?.email ?? "contact@nexuscompta.com"}</a>
                    </div>
                  </div>
      ...
      <footer style={{
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{
            ...
            © {new Date().getFullYear()} {clientName(sessionData) ?? "Nexus Compta."} Tous droits réservés.{/* VILLE_PIED */}{clientCity(sessionData) ? ` · ${clientCity(sessionData)}` : ""}
          </div>
        </div>
      </footer>
