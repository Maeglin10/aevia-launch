...
import { ArrowRight, CheckCircle, Clock, Lock, Mail, MapPin, Phone, Shield, Star } from "lucide-react";
...
/* Serrurier, 2e variante (la 1re est impact-192, urgence). Celle-ci vend la serrurerie de sécurité posée à froid : blindage, A2P, coffres. Signature : GhostSolid — le contour du blindage qui devient matière. Sans photographie. */
...
const ENGAGEMENT = ["Installateurs agréés A2P — matériel certifié CNPP uniquement", "Devis écrit et gratuit avant toute intervention, conformément à l'arrêté du 24 janvier 2017", "Assurance décennale sur les poses, RC professionnelle à jour", "Aucun démarchage, aucune urgence inventée : nos prix ne doublent pas la nuit"];
const TARIFS = [{"a": "Serrure multipoints A2P*", "p": "dès 690 € posée", "n": "Trois points certifiés, cylindre débrayable, clés protégées."}, {"a": "Blindage de porte existante", "p": "dès 1 490 €", "n": "Tôle acier, cornières anti-pinces, multipoints A2P incluse."}, {"a": "Bloc-porte blindé BP1", "p": "dès 2 900 €", "n": "Porte, huisserie et serrure certifiées ensemble — ce que demandent les assurances."}, {"a": "Coffre-fort posé", "p": "sur devis", "n": "Selon classe EN 1143-1 et support. Conseil discret, à l'étude ou chez vous."}];
...
const STATS = [{"value": "A2P", "label": "Serrures certifiées posées"}, {"value": "1 400+", "label": "Portes sécurisées"}, {"value": "10 ans", "label": "Garantie décennale pose"}, {"value": "48 h", "label": "Étude puis devis ferme"}];
...
export default function FaubourgSerruresPage() {
  ...
  const phone = fd?.phone ?? "04 91 00 00 00";
  const telHref = `tel:${fd?.phone ?? "+33491000000"}`;
  const mail = fd?.email ?? "etude@faubourg-serrures.fr";

  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: FONT_BODY, overflowX: "clip" }}>
      <style>{`
        @media (max-width: 900px) { #i332-nav { display: none !important; } .i332-burger { display: flex !important; } }
        @media (max-width: 860px) {
          .i332-hero { grid-template-columns: 1fr !important; padding: 118px 24px 46px !important; gap: 34px !important; }
          .i332-card { max-width: 380px; margin: 0 auto; width: 100%; }
          .i332-split { grid-template-columns: 1fr !important; }
          .i332-stats { grid-template-columns: 1fr 1fr !important; row-gap: 8px; }
          .i332-stats .i332-statcell { border-right: none !important; }
          .i332-pad { padding-left: 24px !important; padding-right: 24px !important; }
          .i332-herotext { padding: 0 24px 44px !important; }
        }
      `}</style>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          {fd?.logoBase64 ? (
            ...
            <>
              ...
              <span style={{ fontFamily: FONT, fontSize: 18, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{fd?.businessName ?? "Faubourg Serrures"}</span>
              <span style={{ fontSize: 10, letterSpacing: 2.2, textTransform: "uppercase", color: C.textMuted, marginLeft: 6 }}>Sécurité</span>
            </>
          )}
        </div>
        <div id="i332-nav" style={{ display: "flex", gap: 24, alignItems: "center" }}>
          {NAV.map(({ l, h }) => (
            <a key={l} href={h} style={{ color: C.textMuted, fontSize: 14, fontWeight: 500, textDecoration: "none", padding: "12px 4px" }}>{l}</a>
          ))}
          <motion.a href="tel:+33491000000" style={{ background: C.accent, color: "#101010", borderRadius: 8, padding: "12px 22px", fontSize: 14, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" }} whileHover={{ scale: 1.03 }}>
            Étude gratuite
          </motion.a>
        </div>
        <button className="i332-burger" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Menu" style={{ display: "none", flexDirection: "column", justifyContent: "center", gap: 5, background: "none", border: "none", cursor: "pointer", padding: 10, minWidth: 44, minHeight: 44 }}>
      ...
      {mobileOpen && (
        <div style={{ position: "fixed", top: 72, left: 0, right: 0, zIndex: 99, background: C.bg, borderBottom: `1px solid ${C.border}`, padding: "20px 28px", display: "flex", flexDirection: "column", gap: 4 }}>
          ...
          <a href="tel:+33491000000" style={{ background: C.accent, color: "#101010", borderRadius: 8, padding: "13px 22px", fontSize: 15, fontWeight: 700, textDecoration: "none", textAlign: "center", marginTop: 8 }}>Étude gratuite</a>
        </div>
      )}

      {/* ── HERO ────────────────────────────────────────────────────────── */}

      <section className="i332-hero" style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "140px 64px 70px", maxWidth: 1080, margin: "0 auto" }}>
        <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", color: C.accent }}>Serrurerie de sécurité · Marseille</span>
      ...
      <section style={{ background: C.bgDark }}>
        <div className="i332-stats i332-pad" style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", maxWidth: 1100, margin: "0 auto", padding: "0 32px" }}>
          {STATS.map((s, idx) => (
            <Reveal key={s.label} delay={idx * 0.08}>
              <div className="i332-statcell" style={{ padding: "30px 8px", textAlign: "center", borderRight: idx < 3 ? "1px solid rgba(255,255,255,0.08)" : "none" }}>
      ...
      {/* ── SERVICES ────────────────────────────────────────────────────── */}
      <section id="services" className="i332-pad" style={{ padding: "96px 64px", background: C.bgSection }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <Reveal>
            <div style={{ marginBottom: 50 }}>
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", color: C.accent }}>Installations</span>
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 3.8vw, 46px)", color: C.text, marginTop: 10, lineHeight: 1.14 }}>
                Ce qui tient<br /><em>quand on force.</em>
              </h2>
            </div>
          </Reveal>
      ...
      {/* ── ENGAGEMENTS ─────────────────────────────────────────────────── */}
      <section id="engagements" className="i332-pad" style={{ padding: "96px 64px", background: C.bgSection }}>
        <div className="i332-split" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 64, alignItems: "center" }}>
          <Reveal>
            <div style={{ borderRadius: 12, border: `1px solid ${C.border}`, background: C.accentLight, aspectRatio: "4/3", justifyContent: "center" , overflow: "hidden" }}><img src={photo(0, "https://images.pexels.com/photos/792034/pexels-photo-792034.jpeg?auto=compress&cs=tinysrgb&w=1400")} alt="Serrure de porte en gros plan" loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /></div>
          </Reveal>
          <Reveal delay={0.15}>
            <div>
              ...
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(26px, 3vw, 40px)", color: C.text, margin: "12px 0 26px", lineHeight: 1.18 }}>
                La sécurité,<br /><em>sans le théâtre.</em>
              </h2>
      ...
      {/* ── TARIFS ──────────────────────────────────────────────────────── */}
      <section id="tarifs" className="i332-pad" style={{ padding: "96px 64px", background: C.bg }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", color: C.accent }}>Tarifs</span>
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 3.5vw, 44px)", color: C.text, marginTop: 10 }}>Des prix <em>posés.</em></h2>
              <p style={{ fontSize: 15, color: C.textMuted, maxWidth: 560, margin: "14px auto 0", lineHeight: 1.7 }}>Pose, déplacement et réglages compris. Les certificats pour votre assureur sont remis avec la facture.</p>
            </div>
          </Reveal>
      ...
      {/* ── AVIS ────────────────────────────────────────────────────────── */}
      <section className="i332-pad" style={{ padding: "96px 64px", background: C.bgDark }}>
        <Reveal>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <h2 style={{ fontFamily: FONT, fontSize: "clamp(26px, 3.4vw, 42px)", color: "#fff" }}>Ils dorment <em style={{ color: C.hi }}>tranquilles</em>.</h2>
          </div>
        </Reveal>
      ...
      {/* ── CONTACT ─────────────────────────────────────────────────────── */}
      <section id="contact" className="i332-pad" style={{ padding: "96px 64px", background: C.accentLight, textAlign: "center" }}>
        <Reveal>
          <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", color: C.accent }}>Étude gratuite</span>
          <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 4vw, 48px)", color: C.text, margin: "14px 0 16px" }}>
            Votre porte mérite<br /><em>un avis d'expert.</em>
          </h2>
          <p style={{ fontSize: 16, color: C.textMuted, maxWidth: 460, margin: "0 auto 36px", lineHeight: 1.7 }}>Étude de sécurité gratuite à domicile, devis ferme sous 48 h. Sans engagement, sans relance.</p>
      ...
      {/* ── FOOTER ──────────────────────────────────────────────────────── */}
      <footer className="i332-pad" style={{ background: C.bgDark, padding: "44px 64px 22px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 28, marginBottom: 30 }}>
            <div>
              <div style={{ fontFamily: FONT, fontSize: 18, color: C.hi, marginBottom: 8 }}>{fd?.businessName ?? "Faubourg Serrures"}</div>
              <p style={{ color: "rgba(255,255,255,0.38)", fontSize: 13, lineHeight: 1.7 }}>Serrurerie de sécurité · Marseille<br />Installateur agréé A2P, assurance décennale</p>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[{ icon: <MapPin size={13} />, t: "Marseille, Bouches-du-Rhône" }, { icon: <Phone size={13} />, t: phone }, { icon: <Clock size={13} />, t: "Lun–Ven 8h30–18h30 · Sam 9h–13h" }].map((item, idx) => (
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.09)", paddingTop: 14, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
            <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 12 }}>
              © 2026 {fd?.businessName ?? "Faubourg Serrures"} — Site réalisé par Aevia WS · SIREN <LegalIdentity />
            </span>
