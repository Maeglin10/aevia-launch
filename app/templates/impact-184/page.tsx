...
export default function BrilloNetPage() {
  ...
  TARIFS = resolveList(
    clientServices(session)?.map((s, i) => ({ ...TARIFS_DEMO[i % TARIFS_DEMO.length], f: s.title, p: s.price ?? "Sur devis", n: s.desc || "" })),
    TARIFS_DEMO,
  );
  ...
  ZONES = resolveList(
    clientAreas(session)?.map((z, i) => ({ v: z, d: "" })) ?? (clientCity(session) ? [{ v: clientCity(session), d: "Sans supplément" }] : undefined),
    ZONES_DEMO,
  );
      ...
      <footer className="bg-[#1c2b2b] pt-20 pb-10 px-6">
        <div className="max-w-[1300px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          {[
            ...
            { t: "Contact", ls: [(clientPhone(sessionData) ?? fd?.phone ?? "04 78 12 34 56"), (clientEmail(sessionData) ?? fd?.email ?? "contact@brillonet.fr"), (clientCity(sessionData) ?? "Lyon") + " Métropole", "7j/7 8h-20h", "Devis gratuit en 2h"] },
          ].map((col, i) => (
            <div key={i}>
