...
let C: Record<string, string> = {
  primary: "var(--brand,#1565c0)",
  primaryLight: "var(--brand-light,#2979d4)",
  primaryDark: "#0d47a1",
  bg: "#111827",
  bgDeep: "#0a0f1a",
  bgCard: "#1f2937",
...
const SERIF = "'Barlow Condensed', sans-serif" as const;
const SANS = "'Open Sans', sans-serif" as const;

const EASE: [number, number, number, number] = [0.16, 1, 0.3, 1];

function PHOTO_LIVE() {
  return {
  hero: (clientPhotos(sessionData)[0] || "https://images.unsplash.com/photo-1517838277536-f5f99be501cd?q=80&w=1600&auto=format&fit=crop"),
  about: (clientPhotos(sessionData)[1] || "https://images.pexels.com/photos/20860617/pexels-photo-20860617.jpeg?auto=compress&cs=tinysrgb&w=1600"),
  special: (clientPhotos(sessionData)[2] || "https://images.unsplash.com/photo-1476480862126-209bfaa8edc8?q=80&w=1600&auto=format&fit=crop"),
  gallery1: (clientPhotos(sessionData)[3] || "https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?q=80&w=800&auto=format&fit=crop"),
  gallery2: (clientPhotos(sessionData)[4] || "https://images.unsplash.com/photo-1517838277536-f5f99be501cd?q=80&w=800&auto=format&fit=crop"),
  gallery3: (clientPhotos(sessionData)[5] || "https://images.unsplash.com/photo-1518611012118-696072aa579a?q=80&w=800&auto=format&fit=crop"),
  gallery4: (clientPhotos(sessionData)[6] || "https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=800&auto=format&fit=crop")
} as const;
}
    ...
    <div style={{
      ...
      >
        <motion.div style={{
          ...
          <img 
            src={PHOTO.hero} 
            alt={`Hero image showing ${clientName(sessionData) ?? "KinéPro Sport Lyon"} core business`} 
      ...
      <section style={{ padding: '80px 24px', background: C.bgDeep, borderBottom: `1px solid ${C.primary}0d` }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{
            ...
            {menuItemsFiltered.length > 0 && [{"value": "100%", "label": "Suivi Individuel"}, {"value": "72h", "label": "Bilan Initial Rapide"}, {"value": "RGE", "label": "Équipements Premium"}].map((stat, i) => (
            ...
            <div>
              <Reveal delay={0.15}>
                ...
                <p style={{
                  ...
                  {clientName(sessionData) ?? "KinéPro Sport"} accompagne les athlètes amateurs et professionnels de {clientCity(sessionData) ?? "la région du Grand Lyon"}. Rééducation post-opératoire, dry needling, thérapie manuelle et retour au sport accéléré.
                </>}</p>
                <p style={{
                  fontSize: 15,
                  lineHeight: 1.7,
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', textAlign: 'center' }}>
          <Reveal>
            ...
            <p style={{
              ...
              Technique de puncture sèche pour libérer les points gâchettes musculaires. Efficace sur les douleurs chroniques et les blessures sportives.
            </p>
          </Reveal>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                  <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                    ...
                    <div>
                      ...
                      <a href={`tel:${(clientPhone(sessionData) ?? fd?.phone ?? "+33527203434").replace(/[^+0-9]/g, "")}`} style={{ fontSize: 15, color: C.text, fontWeight: 700, textDecoration: 'none' }}>{clientPhone(sessionData) ?? fd?.phone ?? "+33 (0)5 00 00 00 00"}</a>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                    ...
                    <div>
                      ...
                      <a href={`mailto:${clientEmail(sessionData) ?? fd?.email ?? "contact@mysite.com"}`} style={{ fontSize: 15, color: C.text, fontWeight: 700, textDecoration: 'none' }}>{clientEmail(sessionData) ?? fd?.email ?? "contact@kinéprosportlyon.com"}</a>
                    </div>
                  </div>
          ...
          <div style={{
            ...
            <div>
              ...
              <p style={{ lineHeight: 1.6, fontSize: 12 }}>
                ...
                Responsable de publication: {clientName(sessionData) ?? "KinéPro Sport Lyon"}<br />
