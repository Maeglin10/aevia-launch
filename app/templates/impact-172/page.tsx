...
export default function LegrandPage() {
  ...
  return (
    <div className="min-h-dvh bg-[#F9F6F0] text-[#1A1510]" style={{ fontFamily: "'Inter', sans-serif" }}>
      <motion.div className="fixed top-0 left-0 h-[2px] bg-[var(--brand,#C9A855)] z-[1000] origin-left" style={{ scaleX: scrollYProgress }} />
      ...
      >
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-10 text-sm text-[#5A5040]">
            ...
            <Link href="#contact" className="ml-2 px-5 py-2.5 bg-[#1A1510] text-[#F9F6F0] text-xs tracking-widest uppercase hover:bg-[var(--brand,#C9A855)] transition-colors duration-300 cursor-pointer">
              Nous contacter
            </Link>
          </div>
              ...
              {["Domaines", "Associés", "Cabinet", "Références", "Contact"].map((l, i) => (
                <motion.div key={l} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }}>
                  <Link href={`#${l.toLowerCase()}`} onClick={() => setMenuOpen(false)}
                    className="text-3xl font-light hover:text-[var(--brand,#C9A855)] transition-colors cursor-pointer"
        ...
        <div className="imx172-hero-content relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 pb-24 pt-32">
          ...
          <Reveal delay={0.3}>
            <div className="flex flex-col sm:flex-row gap-5">
              <Link href="#domaines" className="inline-flex items-center gap-3 px-8 py-4 bg-[var(--brand,#C9A855)] text-[#1A1510] font-medium text-sm tracking-wide uppercase hover:bg-[#E0BC70] transition-colors cursor-pointer">
                Nos domaines <ArrowRight className="w-4 h-4" />
              </Link>
              <Link href="#contact" className="inline-flex items-center gap-3 px-8 py-4 border border-[var(--brand,#C9A855)]/50 text-[#F9F6F0] font-light text-sm tracking-wide uppercase hover:border-[var(--brand,#C9A855)] transition-colors cursor-pointer">
                Prendre contact
              </Link>
            </div>
          </Reveal>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="grid md:grid-cols-5 gap-10 mb-16">
            <div className="md:col-span-2">
              <Reveal>
                <p className="text-xs tracking-[0.25em] uppercase text-[var(--brand,#C9A855)] mb-4">Expertise</p>
              ...
              {DOMAINS.map((d, i) => {
                ...
                return (
                  <button key={d.id} onClick={() => setActiveDomain(i)}
                    ...
                    <div className={`w-10 h-10 flex items-center justify-center flex-shrink-0 ${activeDomain === i ? "text-[var(--brand,#C9A855)]" : "text-[var(--brand,#C9A855)]"}`}>
                      <Icon className="w-5 h-5" />
                    </div>
                    <div>
                      <div className={`font-medium text-sm ${activeDomain === i ? "text-[#F9F6F0]" : "text-[#1A1510]"}`}>{d.label}</div>
                    </div>
                    <ChevronRight className={`w-4 h-4 ml-auto transition-colors ${activeDomain === i ? "text-[var(--brand,#C9A855)]" : "text-[var(--brand,#C9A855)] opacity-0 group-hover:opacity-100"}`} />
                  </button>
                )
              })}
            </div>
            ...
            <div className="lg:col-span-3">
              <AnimatePresence mode="wait">
                <motion.div key={activeDomain} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}
                  ...
                  <div className="w-12 h-12 border border-[var(--brand,#C9A855)] flex items-center justify-center mb-8">
                  ...
                  <Link href="#contact" className="mt-10 inline-flex items-center gap-2 text-sm text-[var(--brand,#C9A855)] border-b border-[var(--brand,#C9A855)] pb-0.5 hover:text-[#1A1510] hover:border-[#1A1510] transition-colors cursor-pointer">
            ...
            {PARTNERS.map((p, i) => (
              <Reveal key={p.name} delay={i * 0.08}>
                <div className="bg-[#1A1510] group cursor-pointer hover:bg-[#231E14] transition-colors duration-300">
                  {/* Gold line top */}
                  <div className="h-[2px] bg-[var(--brand,#C9A855)] relative">
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-[var(--brand,#C9A855)] text-xs tracking-wide uppercase mb-2">{p.title}</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 pt-8 border-t border-[#E8E0D0]">
                  {[{ Icon: Award, text: "Tier 1 Legal 500" }, { Icon: Globe, text: "15 pays couverts" }, { Icon: Briefcase, text: "500+ dossiers/an" }].map(({ Icon, text }) => (
                    <div key={text} className="text-center">
                      <Icon className="w-6 h-6 text-[var(--brand,#C9A855)] mx-auto mb-2" />
            ...
            {REFERENCES.map((r, i) => (
              <Reveal key={r.sector} delay={i * 0.08}>
                <div className="bg-[#E8E0D0] p-8 hover:bg-[#F9F6F0] transition-colors duration-300">
                  <p className="text-xs tracking-widest uppercase text-[var(--brand,#C9A855)] mb-3">{r.sector}</p>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <Reveal key={pub.title} delay={i * 0.08}>
                <div className="border border-[#E8E0D0] p-8 hover:border-[var(--brand,#C9A855)] transition-colors duration-300 cursor-pointer group">
                  <p className="text-xs tracking-widest uppercase text-[var(--brand,#C9A855)] mb-4">{pub.cat}</p>
                  <h3 className="text-base font-light leading-snug mb-6 group-hover:text-[var(--brand,#C9A855)] transition-colors duration-200" style={{ fontFamily: "'Libre Baskerville', serif" }}>
                    {pub.title}
                  </h3>
                  <div className="flex items-center justify-between text-xs text-[#8A7860]">
                    <span>{pub.date}</span>
                    <ArrowRight className="w-4 h-4 text-[var(--brand,#C9A855)] opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
                  </div>
                </div>
              </Reveal>
            ))}
            ...
            <div>
              ...
              <Reveal delay={0.1}>
                <div className="space-y-5">
                  {[{ Icon: MapPin, text: `14 avenue Montaigne, 75008 ${clientCity(sessionData) ?? "Paris"}` }, { Icon: Phone, text: "+33 1 44 20 00 00" }, { Icon: Mail, text: "contact@legrand-associes.fr" }, { Icon: Globe, text: "Également à Bruxelles & Luxembourg" }].map(({ Icon, text }) => (
            ...
            <Reveal delay={0.1}>
              <form className="space-y-6" onSubmit={e => e.preventDefault()}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {["Prénom", "Nom"].map(f => (
                    <div key={f}>
                      ...
                      <input className="w-full bg-transparent border border-[#3A3020] px-4 py-3 text-sm text-[#F9F6F0] focus:outline-none focus:border-[var(--brand,#C9A855)] transition-colors" placeholder={f} />
                    </div>
                  ))}
                </div>
                ...
                <div>
                  ...
                  <select className="w-full bg-[#231E14] border border-[#3A3020] px-4 py-3 text-sm text-[#F9F6F0] focus:outline-none focus:border-[var(--brand,#C9A855)] transition-colors">
                ...
                <div>
                  ...
                  <textarea rows={4} className="w-full bg-transparent border border-[#3A3020] px-4 py-3 text-sm text-[#F9F6F0] focus:outline-none focus:border-[var(--brand,#C9A855)] transition-colors resize-none" placeholder="Décrivez succinctement votre situation et vos besoins..." />
                </div>
                <button type="submit" className="w-full bg-[var(--brand,#C9A855)] text-[#1A1510] py-4 text-xs tracking-widest uppercase font-medium hover:bg-[#E0BC70] transition-colors duration-300 cursor-pointer">
                  Envoyer la demande
                </button>
