...
const LEXIQUE_INTERFACE: Record<string, Record<string, string>> = {
  fr: {
    ...
    /* Titre de COLONNE de pied de page : juste au-dessous vient le lien
       « Mentions légales » lui-même, et la page affichait deux fois la même
       chose. Un intitulé de rubrique, pas un lien. */
...
function traduireLaProse(
  ...
  */
  /*
    L'apostrophe a trois formes et le dictionnaire n'en connaît qu'une.

    Le thème écrit « L&apos;énergie » en JSX, le navigateur rend « L'énergie »
    avec une apostrophe droite, et le rédacteur d'un autre thème a tapé
    « L’énergie » avec la courbe. Une recherche à la lettre échoue sur deux cas
    sur trois. On replie donc les trois formes sur une seule, des deux côtés.
  */
  const plier = (t: string) => t.toLowerCase().replace(/[\u2018\u2019\u02BC\u00B4`]/g, "'");
  const parApostrophe: Record<string, string> = {};
  for (const [k, v] of Object.entries(dico)) parApostrophe[plier(k)] = v;

  const chercher = (t: string) => {
    const direct = dico[t.toLowerCase()] ?? parApostrophe[plier(t)];
    if (direct) return direct;
    if (!renommable || t.trim().length <= nomClient!.trim().length) return undefined;
    const parLaDemo = dico[versLaDemo(t).toLowerCase()] ?? parApostrophe[plier(versLaDemo(t))];
