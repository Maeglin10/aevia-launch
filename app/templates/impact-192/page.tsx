    ...
    <div className="bg-[#0d1524] text-[#f0f4ff] overflow-x-hidden" style={{ fontFamily: "'Exo 2', 'Inter', system-ui, sans-serif" }}>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${scrolled ? "bg-[#0d1524]/98 backdrop-blur-xl py-3 border-b border-[var(--brand,#2563eb)]/15" : "bg-transparent py-7"}`}>
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0388234567").replace(/[^+0-9]/g, "")}`} className="hidden md:flex items-center gap-2 text-[var(--brand,#2563eb)] font-bold text-sm">
              <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "03 88 23 45 67"}
            </a>
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="bg-[#111d30] border-[var(--brand,#2563eb)]/10 p-10">
                <div className="flex flex-col gap-7 mt-16">
                  ...
                  <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0388234567").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 text-[var(--brand,#2563eb)] font-bold text-xl mt-4"><Phone className="w-5 h-5" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "03 88 23 45 67"}</a>
                </div>
              </SheetContent>
            </Sheet>
          </div>
      ...
      <div className="pt-0 fixed top-0 left-0 right-0 z-40 translate-y-[72px]">
        <div className="bg-[var(--brand,#2563eb)] py-2.5 text-center text-[10px] font-bold uppercase tracking-[0.3em] text-white flex items-center justify-center gap-4">
          ...
          <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0388234567").replace(/[^+0-9]/g, "")}`} className="underline ml-2">{clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "03 88 23 45 67"}</a>
        </div>
      </div>
      ...
      <section id="hero" ref={heroRef} className="relative h-[100dvh] min-h-[640px] flex items-end overflow-hidden pt-16">
        ...
        <motion.div style={{ y: heroTextY, opacity: heroOpacity }} className="relative z-10 max-w-[1400px] w-full mx-auto px-6 md:px-12 pb-24">
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.98 }} className="flex flex-wrap gap-4 mb-8">
            <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0388234567").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 px-9 py-4 bg-[var(--brand,#2563eb)] text-white font-bold text-sm uppercase tracking-[0.1em] hover:bg-[#1d4ed8] transition-colors">
              <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "03 88 23 45 67"}
            </a>
      ...
      <section id="urgences" className="py-28 bg-[#111d30]">
        <div className="max-w-[1200px] mx-auto px-6 md:px-12">
          ...
          <Reveal>
            <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0388234567").replace(/[^+0-9]/g, "")}`} className="inline-flex items-center gap-3 px-9 py-4 bg-[var(--brand,#2563eb)] text-white font-bold text-[10px] uppercase tracking-[0.25em] hover:opacity-90 transition-opacity">
              <Phone className="w-4 h-4" /> Ligne d'urgence · {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "03 88 23 45 67"}
            </a>
          </Reveal>
        </div>
      </section>
        ...
        <Reveal>
          <div className="max-w-xl mx-auto px-6">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0388234567").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 px-10 py-4 bg-white text-[var(--brand,#2563eb)] font-bold text-sm hover:bg-[#f0f4ff] transition-colors shadow-lg">
                <Phone className="w-5 h-5" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "03 88 23 45 67"}
              </a>
      ...
      <footer className="bg-[#070d18] pt-16 pb-8 px-6 border-t border-[var(--brand,#2563eb)]/8">
        <div className="max-w-[1300px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          {[
            ...
            { t: "Contact", ls: [(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "03 88 23 45 67"), (fd?.email ?? "contact@securfast.fr"), (clientCity({ formData: fd }) ?? "Strasbourg") + " & 67", "24h/24 — 7j/7", "Devis gratuit"] },
          ].map((col, i) => (
            <div key={i}>
