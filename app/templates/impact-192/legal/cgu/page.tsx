...
export default function Page() {
  return (
    <div className="bg-[#0d1524] min-h-dvh text-[#f0f4ff] font-sans selection:bg-white/10">
      <header className="fixed top-0 left-0 right-0 z-50 px-8 py-6 bg-black/40 backdrop-blur-md border-b border-white/10">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <Link href="/templates/impact-192" className="flex items-center gap-2 text-xs tracking-widest uppercase hover:opacity-80 transition-opacity">
            <ArrowLeft className="w-4 h-4" />
            <span>Retour à l&apos;accueil</span>
          </Link>
          <span className="text-sm font-semibold uppercase tracking-widest text-[#2563eb]">SÉC'URFAST</span>
        </div>
      </header>

      <main className="max-w-4xl mx-auto px-6 pt-36 pb-24">
        <h1 className="text-3xl md:text-5xl font-bold mb-6 text-[#2563eb]">Conditions Générales d'Utilisation</h1>
        ...
        <div className="space-y-8 text-sm leading-relaxed opacity-80 border-t border-white/10 pt-8">
          <section className="space-y-3">
            <h2 className="text-lg font-semibold text-[#2563eb]">1. Éditeur du site</h2>
            <p>Le site web est édité par SÉC'URFAST, société au capital social de 100 000 €, immatriculée au RCS sous le numéro 901 234 567.</p>
          </section>

          <section className="space-y-3">
            <h2 className="text-lg font-semibold text-[#2563eb]">2. Propriété intellectuelle</h2>
          ...
          <section className="space-y-3">
            <h2 className="text-lg font-semibold text-[#2563eb]">3. Protection des données personnelles</h2>
      ...
      <footer className="border-t border-white/10 py-8 px-6 text-center text-xs opacity-40">
        <p>&copy; 2026 SÉC'URFAST. Tous droits réservés.</p>
      </footer>
    </div>
  )
}
