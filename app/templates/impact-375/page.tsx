...
const ENGAGEMENT = ["Devis avant intervention, même en urgence — le prix ne profite pas de la panique", "Garantie décennale sur les poses, verres certifiés CE", "Verre déposé recyclé en filière agréée — le calcin redevient du verre", "Atelier de façonnage sur place : vos mesures ne voyagent pas"];
...
const STATS = [{"value": "7j/7", "label": "Urgence vitrage cassé"}, {"value": "24-48 h", "label": "Remplacement standard"}, {"value": "1/10 mm", "label": "Précision de façonnage"}, {"value": "10 ans", "label": "Décennale sur les poses"}];
...
export default function MiroiterieDuPortPage() {
  ...
  const phone = fd?.phone ?? "02 35 00 00 01";
  const telHref = `tel:${fd?.phone ?? "+33235000001"}`;
  const mail = fd?.email ?? "atelier@miroiterie-du-port.fr";
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          {fd?.logoBase64 ? (
            ...
            <>
              ...
              <span style={{ fontFamily: FONT, fontSize: 18, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{fd?.businessName ?? "Miroiterie du Port"}</span>
...
<section className="i375-hero" style={{ minHeight: "100dvh", display: "grid", gridTemplateColumns: "minmax(0,1.08fr) minmax(0,0.92fr)", gap: 56, alignItems: "center", padding: "140px 64px 70px", maxWidth: 1260, margin: "0 auto" }}>
        <div>
          ...
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.85, ease: [0.16, 1, 0.3, 1] }} style={{ fontFamily: FONT, fontSize: "clamp(34px, 4.6vw, 60px)", color: C.text, lineHeight: 1.1, margin: "18px 0 20px" }}>
            {c?.heroHeadline ?? (<>Le verre, coupé juste,<br /><em style={{ color: C.accentDark }}>posé net.</em></>)}
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }} style={{ fontSize: 16.5, color: C.textMuted, lineHeight: 1.75, maxWidth: 480, marginBottom: 32 }}>
            {c?.heroSubline ?? fd?.tagline ?? "Vitrages cassés remplacés en urgence, double vitrage sur mesure, miroirs, crédences et parois de douche : l'atelier de miroiterie qui façonne sur place, au dixième de millimètre."}
          </motion.p>
        ...
        <div className="i375-card">
          <PushBlur index={i} amount={16}>
            <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden", boxShadow: "0 18px 52px rgba(0,0,0,0.18)" }}>
              <div style={{ aspectRatio: "4/3", background: C.accentLight , overflow: "hidden" }}><img src={photo(0, "https://images.pexels.com/photos/5691531/pexels-photo-5691531.jpeg?auto=compress&cs=tinysrgb&w=1400")} alt="Pose d'un châssis vitré" loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /></div>
        ...
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              ...
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 3.5vw, 44px)", color: C.text, marginTop: 10 }}>Au juste prix <em>du travail d'atelier.</em></h2>
      ...
      <section className="i375-pad" style={{ padding: "96px 64px", background: C.bgDark }}>
        <Reveal>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <h2 style={{ fontFamily: FONT, fontSize: "clamp(26px, 3.4vw, 42px)", color: "#fff" }}>Le verre, <em style={{ color: C.hi }}>sans casse-tête</em>.</h2>
          </div>
        </Reveal>
      ...
      <footer className="i375-pad" style={{ background: C.bgDark, padding: "44px 64px 22px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 28, marginBottom: 30 }}>
            <div>
              <div style={{ fontFamily: FONT, fontSize: 18, color: C.hi, marginBottom: 8 }}>{fd?.businessName ?? "Miroiterie du Port"}</div>
              <p style={{ color: "rgba(255,255,255,0.38)", fontSize: 13, lineHeight: 1.7 }}>Vitrerie · Miroiterie · Le Havre<br />Décennale, atelier de façonnage sur place</p>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[{ icon: <MapPin size={13} />, t: "Le Havre, Seine-Maritime" }, { icon: <Phone size={13} />, t: phone }, { icon: <Clock size={13} />, t: "Lun–Ven 8h–18h · urgence vitrage 7j/7" }].map((item, idx) => (
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.09)", paddingTop: 14, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
            <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 12 }}>
              © 2026 {fd?.businessName ?? "Miroiterie du Port"} — Site réalisé par Aevia WS · SIREN <LegalIdentity />
            </span>
