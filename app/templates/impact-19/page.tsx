...
return (
    <div className="min-h-dvh bg-[#09090B] text-white overflow-x-clip flex flex-col" style={{ fontFamily: "'Barlow', sans-serif" }}>
      ...
      <nav className="fixed top-4 left-4 right-4 z-50">
        <div className="max-w-6xl mx-auto bg-[#09090B]/90 backdrop-blur-md border border-[var(--brand,#C9A86C)]/15 rounded-2xl px-6 py-4 flex items-center justify-between">
          <div onClick={() => goTo("home")} className="cursor-pointer">
            {fd?.logoBase64 ? (
              ...
              <span className="text-[var(--brand,#C9A86C)] tracking-widest" style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.15rem" }}>{fd?.businessName ?? (clientName({ formData: fd }) ?? "Summit Capital")}</span>
            )}
          </div>
          ...
          <button onClick={() => goTo("contact")} className="hidden md:inline-flex border border-[var(--brand,#C9A86C)]/40 text-[var(--brand,#C9A86C)] text-xs tracking-widest uppercase px-5 py-2.5 rounded-xl hover:bg-[var(--brand,#C9A86C)] hover:text-black transition-all cursor-pointer">
            Nous contacter
          </button>
          ...
          <motion.div className="fixed inset-0 z-[100] bg-[#09090B] flex flex-col p-8" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 300, damping: 30 }}>
            <div className="flex items-center justify-between mb-12">
              {fd?.logoBase64 ? (
                ...
                <span className="text-[var(--brand,#C9A86C)] text-xl" style={{ fontFamily: "'Cormorant Garamond', serif" }}>{fd?.businessName ?? (clientName({ formData: fd }) ?? "Summit Capital")}</span>
              )}
            ...
            {[
              ...
              <motion.div key={item.key} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }}>
                <a
                  href={`#${item.key}`}
                  className={`block text-white text-3xl mb-6 cursor-pointer ${page === item.key ? "text-[var(--brand,#C9A86C)] font-bold" : ""}`}
          ...
          <>
            ...
            <section id="hero" ref={heroRef} className="relative min-h-dvh overflow-hidden flex items-center py-24 md:py-0">
              <motion.div className="absolute inset-0 pointer-events-none" style={{ y: heroY }}>
                <Image src={photo(0, "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=1600&q=85")} alt={fd?.businessName ?? (clientName({ formData: fd }) ?? "Summit Capital")} fill className="object-cover opacity-30" priority />
              ...
              <motion.div className="relative z-10 max-w-6xl mx-auto px-6 w-full" style={{ opacity: heroOpacity }}>
                ...
                <Reveal delay={0.3}>
                  <div className="flex gap-4">
                    <button onClick={() => goTo("portefeuille")} className="bg-[var(--brand,#C9A86C)] text-black font-semibold px-8 py-4 rounded-xl hover:bg-[#B8975E] transition-colors cursor-pointer flex items-center gap-2">
                ...
                <div className="grid md:grid-cols-2 gap-5">
                  {theses.slice(0, 2).map((t, i) => (
                    <Reveal key={t.title} delay={i * 0.1}>
                      <div onClick={() => goTo("theses")} className="bg-[#141416] border border-white/5 rounded-2xl p-8 hover:border-[var(--brand,#C9A86C)]/20 transition-colors cursor-pointer group">
                        <div className="w-10 h-10 bg-[var(--brand,#C9A86C)]/10 rounded-xl flex items-center justify-center text-[var(--brand,#C9A86C)] mb-5 group-hover:bg-[var(--brand,#C9A86C)] group-hover:text-black transition-colors">{t.icon}</div>
      ...
      <footer className="bg-[#09090B] border-t border-white/5 py-12 px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6 text-xs text-white/20">
          <div onClick={() => goTo("home")} className="text-[var(--brand,#C9A86C)] text-lg cursor-pointer" style={{ fontFamily: "'Cormorant Garamond', serif" }}>{fd?.businessName ?? (clientName({ formData: fd }) ?? "Summit Capital")}</div>
    ...
    <section id="realisations" className="py-20 px-6 bg-[#0F0F11] border-t border-white/5">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {theses.map((t, i) => (
            <div key={t.title} className="bg-[#141416] border border-white/5 rounded-2xl p-8 hover:border-[var(--brand,#C9A86C)]/20 transition-all cursor-pointer">
              <div className="w-12 h-12 bg-[var(--brand,#C9A86C)]/10 rounded-xl flex items-center justify-center text-[var(--brand,#C9A86C)] mb-6">{t.icon}</div>
          ...
          <div className="flex gap-2 flex-wrap justify-center mt-8">
            {sectors.map(s => (
              <button
                ...
                className={`px-4 py-2 text-xs transition-all cursor-pointer rounded-lg border font-medium ${activeFilter === s ? "bg-[var(--brand,#C9A86C)] text-black border-[var(--brand,#C9A86C)]" : "border-white/10 text-white/40 hover:border-[var(--brand,#C9A86C)]/40"}`}
              >
                {s}
              </button>
            ))}
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((co, i) => (
              <motion.div
                ...
                className="bg-[#141416] border border-white/5 rounded-2xl p-6 hover:border-[var(--brand,#C9A86C)]/20 transition-all cursor-pointer"
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="w-12 h-12 bg-[var(--brand,#C9A86C)]/10 rounded-xl flex items-center justify-center text-[var(--brand,#C9A86C)] text-lg font-bold" style={{ fontFamily: "'Cormorant Garamond', serif" }}>{co.name.charAt(0)}</div>
                  <span className="text-[var(--brand,#C9A86C)] text-xs border border-[var(--brand,#C9A86C)]/20 px-2.5 py-1 rounded-full">{co.round}</span>
                </div>
    ...
    <section id="services" className="py-20 px-6 bg-[#0F0F11] border-t border-white/5">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {team.map((t, i) => (
            <div key={t.name} className="bg-[#141416] border border-white/5 rounded-2xl p-8 hover:border-[var(--brand,#C9A86C)]/20 transition-all flex flex-col justify-between">
              <div>
                <div className="w-16 h-16 bg-[var(--brand,#C9A86C)] rounded-2xl mb-6 flex items-center justify-center text-black text-2xl font-bold" style={{ fontFamily: "'Cormorant Garamond', serif" }}>{t.name.charAt(0)}</div>
                <h3 className="text-white text-2xl mb-1" style={{ fontFamily: "'Cormorant Garamond', serif" }}>{t.name}</h3>
                <p className="text-[var(--brand,#C9A86C)] text-xs tracking-widest uppercase mb-4">{t.role}</p>
        ...
        <div className="mt-20 border-t border-white/10 pt-16">
          ...
          <div className="relative border-l border-[var(--brand,#C9A86C)]/20 pl-10 space-y-10 max-w-xl mx-auto">
            {milestones.map((m, i) => (
              <div key={m.year} className="relative">
                <div className="absolute -left-[2.875rem] w-3 h-3 rounded-full bg-[var(--brand,#C9A86C)] border-2 border-[#09090B] top-1" />
                <span className="text-[var(--brand,#C9A86C)] text-xs tracking-widest uppercase">{m.year} · {m.label}</span>
    ...
    <section className="py-20 px-6 bg-[#09090B] border-t border-white/5">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
          {notes.map((n, idx) => (
            <div key={idx} className="bg-[#141416] border border-white/5 rounded-2xl p-8 hover:border-[var(--brand,#C9A86C)]/20 transition-all cursor-pointer">
              <span className="text-xs text-[var(--brand,#C9A86C)] font-mono">{n.date}</span>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid md:grid-cols-12 gap-8 items-start">
          ...
          <div className="md:col-span-7 bg-[#141416] p-8 rounded-2xl border border-white/10 flex flex-col gap-4">
            <input type="text" placeholder="Nom de la startup" className="bg-[#1E1E22] border border-white/10 text-white text-sm px-4 py-3.5 rounded-xl outline-none focus:border-[var(--brand,#C9A86C)]/50 placeholder-white/30" />
            <input type="email" placeholder="Email fondateur" className="bg-[#1E1E22] border border-white/10 text-white text-sm px-4 py-3.5 rounded-xl outline-none focus:border-[var(--brand,#C9A86C)]/50 placeholder-white/30" />
            <select className="bg-[#1E1E22] border border-white/10 text-white/60 text-sm px-4 py-3.5 rounded-xl outline-none focus:border-[var(--brand,#C9A86C)]/50 cursor-pointer">
            ...
            <textarea rows={4} placeholder="Décrivez votre projet en quelques phrases (problématique, technologie clé, clients cibles...)" className="bg-[#1E1E22] border border-white/10 text-white text-sm px-4 py-3.5 rounded-xl outline-none focus:border-[var(--brand,#C9A86C)]/50 placeholder-white/30 resize-none" />
            <button className="bg-[var(--brand,#C9A86C)] text-black font-semibold px-6 py-4 rounded-xl hover:bg-[#B8975E] transition-colors cursor-pointer text-sm flex items-center justify-center gap-2">
    ...
    <section id="contact" className="py-20 px-6 bg-[#09090B] border-t border-white/5 font-mono text-xs text-white/40">
      <div className="max-w-3xl mx-auto space-y-12">
        ...
        <div className="border border-[var(--brand,#C9A86C)]/20 bg-[#141416] p-8 rounded-2xl space-y-6">
          ...
          <div className="border-b border-white/5 pb-4">
            <div className="text-[var(--brand,#C9A86C)] text-[10px] font-bold uppercase mb-2">HÉBERGEUR</div>
            <p className="leading-relaxed font-sans text-sm text-white/75">
