          ...
          <div className="hidden md:flex items-center gap-8 text-white/50 text-sm font-medium">
            {[
              ...
              <Link
                ...
                className={`hover:text-[#C9A86C] transition-colors cursor-pointer ${"confidentialite" === item.key ? "text-[#C9A86C] font-bold" : ""}`}
              >
                {item.name}
              </Link>
            ))}
          ...
          <motion.div className="fixed inset-0 z-[100] bg-[#09090B] flex flex-col p-8" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 300, damping: 30 }}>
            ...
            {[
              ...
              <motion.div key={item.key} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }}>
                <Link
                  href={`/templates/impact-19/${item.key}`}
                  className={`block text-white text-3xl mb-6 cursor-pointer ${"confidentialite" === item.key ? "text-[#C9A86C] font-bold" : ""}`}
      ...
      <main className="flex-grow pt-32">
        ...
        <section className="py-20 px-6 bg-[#09090B] border-t border-white/5 font-mono text-xs text-white/40">
          <div className="max-w-3xl mx-auto space-y-12">
            <div>
              <span className="text-[#C9A86C] text-[10px] uppercase tracking-widest mb-3 block font-bold">Sécurité des données</span>
              <h1 className="text-4xl md:text-5xl font-light uppercase text-white mb-8" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Politique de Confidentialité</h1>
            </div>
            <div className="border border-[#C9A86C]/20 bg-[#141416] p-8 rounded-2xl space-y-6">
              <div className="border-b border-white/5 pb-4">
                <div className="text-[#C9A86C] text-[10px] font-bold uppercase mb-2">COLLECTE DES DONNÉES</div>
                <p className="leading-relaxed font-sans text-sm text-white/75">
                  Les données personnelles collectées via notre formulaire de contact ou lors de la soumission de pitch decks sont utilisées exclusivement dans le cadre de l'évaluation des opportunités d'investissement. Ces données sont conservées de manière sécurisée et ne sont en aucun cas revendues à des tiers.
                </p>
              </div>
              <div className="border-b border-white/5 pb-4">
                <div className="text-[#C9A86C] text-[10px] font-bold uppercase mb-2">DURÉE DE CONSERVATION</div>
                <p className="leading-relaxed font-sans text-sm text-white/75">
                  Conformément au RGPD, nous conservons vos données pour la durée nécessaire au traitement de votre dossier. Si aucune suite n'est donnée, vos données sont supprimées ou anonymisées dans un délai de 24 mois après le dernier contact.
                </p>
              </div>
              <div>
