      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${scrolled ? "bg-[#fefcf8]/98 backdrop-blur-xl py-3 shadow-sm border-b border-[var(--brand,#d4a853)]/12" : "bg-transparent py-7"}`}>
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0478345678").replace(/[^+0-9]/g, "")}`} className="hidden md:flex items-center gap-2 text-[var(--brand,#d4a853)] font-bold text-sm">
              <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "04 78 34 56 78"}
            </a>
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="bg-[#fefcf8] border-slate-100 p-10">
                <div className="flex flex-col gap-7 mt-16">
                  ...
                  <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0478345678").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 text-[var(--brand,#d4a853)] font-bold text-xl mt-4"><Phone className="w-5 h-5" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "04 78 34 56 78"}</a>
                </div>
              </SheetContent>
            </Sheet>
          </div>
      ...
      <section id="hero" ref={heroRef} className="relative h-[100dvh] min-h-[640px] flex [align-items:safe_flex-end] overflow-hidden pt-24 sm:pt-0">
        ...
        <motion.div style={{ y: heroTextY, opacity: heroOpacity }} className="relative z-10 max-w-[1400px] w-full mx-auto px-6 md:px-12 pb-30">
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.0 }} className="flex flex-wrap gap-4 mb-8">
            ...
            <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0478345678").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 px-9 py-4 border border-[#fefcf8]/12 text-[#fefcf8]/38 font-bold text-[10px] uppercase tracking-widest hover:border-[var(--brand,#d4a853)]/40 hover:text-[var(--brand,#d4a853)] transition-all">
              <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "04 78 34 56 78"}
            </a>
          </motion.div>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
          <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2.4 }} className="w-[1px] h-10 bg-gradient-to-b from-[var(--brand,#d4a853)]/50 to-transparent" />
        </div>
      </section>
      ...
      <section id="formules" className="py-28 bg-[#fefcf8]">
        <div className="max-w-[1300px] mx-auto px-6 md:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {PRESTATIONS.map((p, i) => (
              <Reveal key={i} delay={i * 0.07}>
                <div className="group p-8 bg-white border border-[#f0e8d5] hover:border-[var(--brand,#d4a853)]/30 hover:shadow-xl hover:shadow-[var(--brand,#d4a853)]/5 transition-all duration-500 h-full">
                  <div className="w-10 h-10 bg-[var(--brand,#d4a853)]/8 flex items-center justify-center mb-5 group-hover:bg-[var(--brand,#d4a853)] transition-colors duration-500">
                    <p.icon className="w-5 h-5 text-[var(--brand,#d4a853)] group-hover:text-white transition-colors" />
                  </div>
                  <h3 className="font-bold text-[#1f1d1a] mb-3 group-hover:text-[var(--brand,#d4a853)] transition-colors" style={{ fontFamily: "'Playfair Display', serif" }}>{p.title}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {[
              ...
              <Reveal key={i} delay={i * 0.1}>
                <div className="p-8 border border-[#fefcf8]/6 hover:border-[var(--brand,#d4a853)]/20 transition-colors h-full flex flex-col">
                  <div className="flex gap-1 mb-5">
                    {[...Array(5)].map((_, j) => <Star key={j} className="w-4 h-4 fill-[var(--brand,#d4a853)] text-[var(--brand,#d4a853)]" />)}
                  </div>
                  ...
                  <div className="mt-6 pt-5 border-t border-[#fefcf8]/6">
                    ...
                    <div className="text-[10px] text-[var(--brand,#d4a853)] mt-1 flex items-center gap-1"><MapPin className="w-3 h-3" />{t.l}</div>
                  </div>
                </div>
              </Reveal>
            ))}
        ...
        <Reveal>
          <div className="max-w-xl mx-auto px-6">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              ...
              <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0478345678").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 px-10 py-4 border border-white/25 text-white font-bold text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all">
                <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "04 78 34 56 78"}
              </a>
            </div>
          </div>
      ...
      <footer className="bg-[#140f0a] pt-20 pb-10 px-6">
        <div className="max-w-[1300px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          {[
            ...
            { t: "Contact", ls: [(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "04 78 34 56 78"), (fd?.email ?? "contact@table-exception.fr"), (clientCity({ formData: fd }) ?? "Lyon") + " & Rhône-Alpes", "Lun-Sam 8h-19h", "Devis sous 24h"] },
          ].map((col, i) => (
            <div key={i}>
