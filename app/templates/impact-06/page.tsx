...
export default function NeuralisPage() {
  ...
  return (
    <div className="premium-theme min-h-dvh bg-[#020204] text-[#e0e0e0] font-sans selection:bg-[var(--brand,#00f2ff)] selection:text-black overflow-x-hidden">
        ...
        <div className="max-w-[1600px] mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-12 text-[10px] font-bold uppercase tracking-[0.3em] text-white/40">
            {["Innovations", "Solutions", "Ethics", "Atelier", "Support"].map(link => (
              <Link key={link} href="#solutions" className="hover:text-[var(--brand,#00f2ff)] transition-colors cursor-pointer">{link}</Link>
            ))}
          </div>

          <div className="flex items-center gap-8">
             <button className="hidden md:flex items-center gap-3 group">
                <span className="text-[10px] font-bold uppercase tracking-widest text-white/60 group-hover:text-[var(--brand,#00f2ff)] transition-colors">Client_Portal</span>
                <div className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center text-white/40 group-hover:bg-[var(--brand,#00f2ff)] group-hover:text-black group-hover:border-[var(--brand,#00f2ff)] transition-all">
                   <Scan className="w-4 h-4" />
                </div>
             </button>
             <button onClick={() => setMenuOpen(true)} className="lg:hidden text-[var(--brand,#00f2ff)]"><Menu className="w-6 h-6" /></button>
          </div>
        </div>
      </nav>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.1 }} className="fixed inset-0 z-[100] bg-[#020204] p-12 flex flex-col justify-center gap-10">
             <button onClick={() => setMenuOpen(false)} className="absolute top-10 right-8 text-white/40 hover:text-[var(--brand,#00f2ff)]"><X className="w-10 h-10"/></button>
             <div className="flex flex-col gap-6 text-6xl font-black uppercase text-white/5">
                {["Products", "Research", "Labs", "Contact"].map(l => (
                   <Link key={l} href="#solutions" onClick={() => setMenuOpen(false)} className="hover:text-[var(--brand,#00f2ff)] transition-all hover:pl-6">{l}</Link>
                ))}
             </div>
          </motion.div>
        )}
      ...
      <section id="hero" className="relative h-[100svh] flex items-center overflow-hidden pt-36 md:pt-0">
        ...
        <div className="relative z-10 max-w-[1600px] mx-auto px-6 md:px-12 w-full grid grid-cols-1 lg:grid-cols-2 items-center gap-20">
          <Reveal>
             <h1 style={{ /* TITRE_DEGAGE */ marginTop: 301 }} className="text-5xl sm:text-6xl md:text-[11rem] font-black leading-[0.9] md:leading-[0.8] tracking-tight md:tracking-tighter mb-12 uppercase text-white break-words">{<>Tame Your<br/> <span className="text-[var(--brand,#00f2ff)]">Biology.</span>
             </>}</h1>
             ...
             <div className="flex flex-col sm:flex-row gap-6">
                <MagneticBtn className="px-12 py-5 bg-[var(--brand,#00f2ff)] text-black text-[10px] font-black uppercase tracking-[0.3em] rounded-full hover:scale-105 transition-all cursor-pointer shadow-[0_0_50px_rgba(0,242,255,0.3)]">
                  Enter The Atelier
                </MagneticBtn>
          ...
          <div className="hidden lg:flex justify-end relative">
             <Reveal delay={0.4}>
                <div className="relative w-[500px] h-[500px] rounded-full border border-[var(--brand,#00f2ff)]/10 flex items-center justify-center">
                 ...
                 <div className="group space-y-10 cursor-pointer" onMouseEnter={() => setActiveProduct(item.id)} onMouseLeave={() => setActiveProduct(null)}>
                    <div className="relative aspect-square overflow-hidden rounded-sm grayscale group-hover:grayscale-0 transition-all duration-[1s]">
                       ...
                       <AnimatePresence>
                          {activeProduct === item.id && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex items-center justify-center bg-[var(--brand,#00f2ff)]/10 backdrop-blur-[2px]">
                    ...
                    <div className="space-y-6">
                       <div className="flex justify-between items-baseline">
                          <h3 className="text-4xl font-black uppercase tracking-tighter text-white italic group-hover:text-[var(--brand,#00f2ff)] transition-colors">{item.name}</h3>
                          <span className="text-lg font-black text-[var(--brand,#00f2ff)] tracking-tighter">{item.price}</span>
                       </div>
                       ...
                       <div className="flex items-center gap-4">
                          ...
                          <Sparkles className="w-5 h-5 text-white/10 group-hover:text-[var(--brand,#00f2ff)] transition-all" />
                       </div>
                    </div>
                 </div>
              </Reveal>
      ...
      <section className="py-40 bg-[#040408] overflow-hidden relative border-t border-white/5">
         <div className="absolute top-0 right-0 w-[40rem] h-[40rem] bg-[var(--brand,#00f2ff)]/5 blur-[120px] rounded-full" />
         <div className="max-w-[1600px] mx-auto px-6 md:px-12">
            <Reveal>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
               {INNOVATIONS.map((s, i) => (
                 <Reveal key={i} delay={i * 0.1}>
                    <div className="p-16 border border-white/5 bg-white/[0.01] hover:border-[var(--brand,#00f2ff)]/30 transition-all group h-full flex flex-col relative overflow-hidden">
                       <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center text-[var(--brand,#00f2ff)] mb-10 group-hover:bg-[var(--brand,#00f2ff)] group-hover:text-black transition-all duration-500">
      ...
      <section className="py-40 px-6 md:px-12 bg-black">
         <div className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
            ...
            <Reveal delay={0.2}>
               <span className="text-[10px] font-bold uppercase tracking-[0.5em] text-[var(--brand,#00f2ff)] mb-8 block">The Protocol</span>
               <h2 className="text-6xl md:text-9xl font-black italic tracking-tighter leading-[0.8] mb-12 uppercase text-white">{c?.aboutTitle ?? fd?.businessName ?? <>
                 Pure <br/> <span className="text-[var(--brand,#00f2ff)] not-italic">Sync.</span>
               </>}</h2>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  {[
                    ...
                    <div key={i} className="space-y-4">
                       <val.icon className="w-6 h-6 text-[var(--brand,#00f2ff)]" />
               ...
               <MagneticBtn className="mt-20 px-14 py-6 bg-white text-black text-[10px] font-black uppercase tracking-[0.4em] rounded-full hover:bg-[var(--brand,#00f2ff)] hover:text-black transition-all shadow-2xl">
        ...
        <div className="max-w-[1600px] mx-auto px-6 md:px-12">
           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
              {[
                ...
                <div key={i} className="p-12 border border-white/5 bg-white/[0.01] hover:border-[var(--brand,#00f2ff)]/20 transition-all group">
                   <Badge className="bg-[var(--brand,#00f2ff)]/10 text-[var(--brand,#00f2ff)] border-[var(--brand,#00f2ff)]/20 mb-8">{trial.phase}</Badge>
                   <h3 className="text-2xl font-black uppercase italic text-white mb-4 group-hover:text-[var(--brand,#00f2ff)] transition-colors">{trial.focus}</h3>
                   <div className="text-4xl font-black text-[var(--brand,#00f2ff)] mb-6 italic tracking-tighter">{trial.result}</div>
              ...
              {[
                ...
                { city: "Stockholm", status: "Nominal", latency: "0.4ms" },
                 ...
                 <Reveal key={i} delay={i * 0.05}>
                    <div className="p-8 border border-white/5 bg-white/[0.01] hover:border-[var(--brand,#00f2ff)]/30 transition-all group">
                       <div className="flex justify-between items-start mb-6">
                          <span className="text-lg font-black text-white italic group-hover:text-[var(--brand,#00f2ff)] transition-colors">{node.city}</span>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
                    ...
                    {[
                      ...
                      { q: "Cognitive Liberty", a: "No external overrides. Our firmware is open-source and audited by third-party ethics councils." },
                       ...
                       <div key={i} className="group border-b border-white/5 pb-8">
                          <h4 className="text-lg font-black uppercase italic text-white mb-4 group-hover:text-[var(--brand,#00f2ff)] transition-colors">{item.q}</h4>
              ...
              <Reveal delay={0.2}>
                 <div className="relative aspect-square bg-white/[0.02] rounded-sm border border-white/5 p-16 flex flex-col justify-center">
                    <div className="mb-12">
                       <ShieldCheck className="w-16 h-16 text-[var(--brand,#00f2ff)] mb-8" />
      ...
      <section id="realisations" className="py-40 bg-black overflow-hidden relative">
        <div className="absolute top-0 right-0 w-[50rem] h-[50rem] bg-[var(--brand,#00f2ff)]/5 blur-[150px] rounded-full" />
        <div className="max-w-[1600px] mx-auto px-6 md:px-12">
           <Reveal>
              <div className="mb-32 text-center">
                ...
                <Reveal key={i} delay={i * 0.2}>
                   <div className="space-y-12">
                      ...
                      <div>
                         ...
                         <span className="text-[10px] font-bold uppercase tracking-[0.4em] text-[var(--brand,#00f2ff)]">{item.role}</span>
                      </div>
                   </div>
                </Reveal>
              ))}
           ...
           <Accordion type="single" collapsible className="w-full space-y-4">
              {[
                ...
                <AccordionItem key={i} value={`item-${i}`} className="border border-white/5 bg-white/[0.02] px-8 rounded-sm">
                   <AccordionTrigger className="text-[11px] font-black uppercase tracking-[0.3em] text-white hover:text-[var(--brand,#00f2ff)] py-8 no-underline italic">
                      {item.q}
                   </AccordionTrigger>
      ...
      <footer className="bg-[#020204] pt-40 pb-16 px-6 md:px-12 border-t border-white/5">
        <div className="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-32 mb-40">
              ...
              <Reveal>
                 ...
                 <div className="flex gap-6">
                    {[Globe, Globe, Mail].map((Icon, i) => (
                      <button key={i} className="w-14 h-14 rounded-full border border-white/10 flex items-center justify-center text-white/40 hover:bg-[var(--brand,#00f2ff)] hover:text-black hover:border-[var(--brand,#00f2ff)] transition-all">
           ...
           <div className="lg:col-span-2">
              <h4 className="text-[11px] font-black uppercase tracking-widest text-[var(--brand,#00f2ff)] mb-12">Innovations</h4>
           ...
           <div className="lg:col-span-2">
              <h4 className="text-[11px] font-black uppercase tracking-widest text-[var(--brand,#00f2ff)] mb-12">Experience</h4>
              <ul className="space-y-6 text-[10px] font-bold uppercase tracking-widest text-white/30">
           ...
           <div className="lg:col-span-2">
              <h4 className="text-[11px] font-black uppercase tracking-widest text-[var(--brand,#00f2ff)] mb-12">Studio</h4>
              <ul className="space-y-6 text-[10px] font-bold uppercase tracking-widest text-white/30">
