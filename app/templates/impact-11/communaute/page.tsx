...
export default function Page() {
  ...
  const PageContent = () => {
    return (
      <section className="py-20 px-6 bg-white border-t border-gray-100 min-h-[70vh]">
        <div className="max-w-4xl mx-auto text-center">
          <div className="w-20 h-20 bg-[#5b48c9]/10 rounded-2xl flex items-center justify-center mx-auto mb-8">
            <Users className="w-10 h-10 text-[#5b48c9]" />
          </div>
          <h1 className="text-4xl md:text-6xl font-bold mb-6">La Communauté EduPath</h1>
          <p className="text-xl text-gray-500 mb-12">
            Rejoignez plus de 250 000 apprenants et mentors qui s'entraident au quotidien pour faire avancer leur carrière.
          </p>

          <div className="grid md:grid-cols-2 gap-8 text-left mb-16">
            <div className="bg-gray-50 rounded-3xl p-8 border border-gray-100 hover:shadow-lg transition-all">
              <MessageSquare className="w-8 h-8 text-[#0EA5E9] mb-4" />
              <h3 className="text-2xl font-bold mb-3">Discord Privé</h3>
              <p className="text-gray-600 mb-6">
                Échangez en direct avec les autres étudiants de votre promotion, posez vos questions aux mentors et participez aux sessions de pair-programming.
              </p>
              <button className="bg-[#0EA5E9] text-white px-6 py-3 rounded-xl font-semibold hover:bg-[#0284C7] transition-colors">
                Rejoindre le Discord
              </button>
            </div>
            <div className="bg-gray-50 rounded-3xl p-8 border border-gray-100 hover:shadow-lg transition-all">
              <Globe className="w-8 h-8 text-[#5b48c9] mb-4" />
              <h3 className="text-2xl font-bold mb-3">Événements en ligne</h3>
              <p className="text-gray-600 mb-6">
                Chaque mois, assistez à nos masterclasses exclusives, webinars et hackathons animés par des experts de l'industrie de la tech.
              </p>
              <button className="bg-[#5b48c9] text-white px-6 py-3 rounded-xl font-semibold hover:bg-[#4a3aa8] transition-colors">
                Voir l'agenda
              </button>
            </div>
          </div>
...
// @ts-ignore
            <Link href="/templates/impact-11/cours" className={`hover:text-[#5b48c9] transition-colors cursor-pointer ${"communaute" === "cours" ? "text-[#5b48c9] font-bold" : ""}`}>Cours</Link>
// @ts-ignore
            <Link href="/templates/impact-11/mentoring" className={`hover:text-[#5b48c9] transition-colors cursor-pointer ${"communaute" === "mentoring" ? "text-[#5b48c9] font-bold" : ""}`}>Mentoring</Link>
// @ts-ignore
            <Link href="/templates/impact-11/tarifs" className={`hover:text-[#5b48c9] transition-colors cursor-pointer ${"communaute" === "tarifs" ? "text-[#5b48c9] font-bold" : ""}`}>Tarifs</Link>
          </div>
...
// @ts-ignore
                  <Link href="/templates/impact-11/cours" className={`text-xl hover:text-[#5b48c9] transition-colors ${"communaute" === "cours" ? "text-[#5b48c9] font-bold" : "text-gray-600"}`}>Cours</Link>
// @ts-ignore
                  <Link href="/templates/impact-11/mentoring" className={`text-xl hover:text-[#5b48c9] transition-colors ${"communaute" === "mentoring" ? "text-[#5b48c9] font-bold" : "text-gray-600"}`}>Mentoring</Link>
// @ts-ignore
                  <Link href="/templates/impact-11/tarifs" className={`text-xl hover:text-[#5b48c9] transition-colors ${"communaute" === "tarifs" ? "text-[#5b48c9] font-bold" : "text-gray-600"}`}>Tarifs</Link>
// @ts-ignore
                  <Link href="/templates/impact-11/legal" className={`text-xl hover:text-[#5b48c9] transition-colors ${"communaute" === "legal" ? "text-[#5b48c9] font-bold" : "text-gray-600"}`}>Mentions Légales</Link>
               </div>
            </SheetContent>
         </Sheet>
        </div>
