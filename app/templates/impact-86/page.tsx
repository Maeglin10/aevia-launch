...
export default function AuraWellnessPage() {
  ...
  useEffect(() => {
    ...
    let n = 3;
    const _photoArrays: any[] = [blogArticles];
          ...
          <Reveal>
            <div className="bg-[#EDE9E2] rounded-3xl border border-[#D8D0C4] p-8 md:p-10 h-full">
              ...
              <ul className="space-y-5 text-sm text-[#6B5E52]">
                ...
                <li className="flex items-center gap-3">
                  <Mail className="w-5 h-5 text-[var(--brand,#7C9E87)] shrink-0" />{clientEmail(sessionData) ?? fd?.email ?? "contact@aurawellness.fr"}</li>
              </ul>
            </div>
          </Reveal>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-4 gap-10">
          <div>
            ...
            <div className="space-y-1 text-xs text-[#6B5E52]">
              ...
              <div className="flex items-center gap-2"><Mail className="w-3 h-3" />{clientEmail(sessionData) ?? fd?.email ?? "contact@aurawellness.fr"}</div>
            </div>
          </div>
          {[
