                ...
                <>
                  ...
                  <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <div>
                      ...
                      <input id="bk-phone" type="tel" required value={phone} onChange={(e) => setPhone(e.target.value)} className={inputCls} placeholder="06 21 58 58 58" />
                    </div>
                    <div>
    ...
    <div className="bg-[#0a0908] text-[#f5f0e8] overflow-x-hidden" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${scrolled ? "bg-[#0a0908]/97 backdrop-blur-xl py-3 border-b border-[var(--brand,#c9a84c)]/10" : "bg-transparent py-7"}`}>
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0556789012").replace(/[^+0-9]/g, "")}`} className="hidden md:flex items-center gap-2 text-[var(--brand,#c9a84c)] font-bold text-sm" style={{ fontFamily: "'DM Mono', monospace" }}>
              <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "05 56 78 90 12"}
            </a>
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="bg-[#0f0e0c] border-[var(--brand,#c9a84c)]/10 p-10">
                <div className="flex flex-col gap-7 mt-16">
                  ...
                  <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0556789012").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 text-[var(--brand,#c9a84c)] font-bold text-lg mt-4"><Phone className="w-5 h-5" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "05 56 78 90 12"}</a>
                </div>
              </SheetContent>
            </Sheet>
          </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    { icon: Calendar, t: "Résa en ligne ou " + (clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "05 56 78 90 12") },
                  ].map((item, i) => (
      ...
      <footer className="bg-[#050403] pt-16 pb-8 px-6 border-t border-[var(--brand,#c9a84c)]/8">
        <div className="max-w-[1300px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          {[
            ...
            { t: "Adresse", ls: [(clientAddress(sessionData) ?? "12 rue du Pas Saint-Georges"), clientCodePostalVille(sessionData, "33000", "Bordeaux"), "Mar-Sam 9h–19h", (clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "05 56 78 90 12"), (fd?.email ?? "contact@gentlemanscut.fr")] },
          ].map((col, i) => (
            <div key={i}>
