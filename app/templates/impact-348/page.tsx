...
const HERO = [
  { k: "Métiers de bouche", sub: "Le geste appris en cuisine réelle, pas en amphi.", tiles: [{ icon: ChefHat, t: "CAP Cuisine en 8 mois", d: "Cuisine pédagogique pro, 12 par promotion, 8 semaines de stage.", bg: "#f6e5da", fg: "var(--brand, #7e3f20)" }, { icon: Compass, t: "Titre Pro Boulanger", d: "Fournil sur place, four à sole, épreuves en conditions réelles.", bg: "#191410", fg: "#f6e5da" }, { icon: ChefHat, t: "HACCP inclus", d: "La formation hygiène obligatoire, comprise dans chaque parcours.", bg: "#f1ebe2", fg: "var(--brand, #7e3f20)" }] },
  { k: "Métiers du bois", sub: "L'atelier de 400 m², cœur de l'école.", tiles: [{ icon: Hammer, t: "Titre Pro Menuisier", d: "Machines pro, projets réels livrés à des clients solidaires.", bg: "#191410", fg: "#f6e5da" }, { icon: Compass, t: "Ébénisterie d'art", d: "Module de perfectionnement : placage, marqueterie, finitions.", bg: "#f6e5da", fg: "var(--brand, #7e3f20)" }, { icon: Hammer, t: "Sécurité machines", d: "Habilitations passées à l'atelier, sur les vraies machines.", bg: "#f1ebe2", fg: "var(--brand, #7e3f20)" }] },
  { k: "Numérique", sub: "Le code, appris en construisant.", tiles: [{ icon: Laptop, t: "Développeur web (RNCP 5)", d: "9 mois, projets réels, jury de professionnels en poste.", bg: "#f1ebe2", fg: "var(--brand, #7e3f20)" }, { icon: Compass, t: "Alternance possible", d: "Rythme 1 semaine école / 3 semaines entreprise dès le 3e mois.", bg: "#f6e5da", fg: "var(--brand, #7e3f20)" }, { icon: Laptop, t: "Matériel prêté", d: "Un portable confié à chaque stagiaire pour toute la durée.", bg: "#191410", fg: "#f6e5da" }] }
];

const SERVICES_SOURCE = [{"titre": "CAP Cuisine (8 mois)", "desc": "En cuisine pédagogique professionnelle, avec 8 semaines de stage en restaurant. Le CAP en candidat libre, préparé sérieusement.", "tag": "Cuisine"}, {"titre": "Titre Pro Menuisier fabricant", "desc": "Niveau 3 RNCP, dans un atelier de 400 m². Les projets des stagiaires équipent des associations locales — du réel, jusqu'au client.", "tag": "Bois"}, {"titre": "Développeur web et web mobile", "desc": "Titre RNCP niveau 5 en 9 mois. Front, back, bases de données, projet final soutenu devant un jury de développeurs en poste.", "tag": "Code"}, {"titre": "Boulanger (Titre Pro)", "desc": "Fournil sur place, levains menés chaque nuit de formation, épreuves blanches en conditions d'examen réelles.", "tag": "Fournil"}, {"titre": "Bilan de compétences", "desc": "24 h sur 8 semaines pour choisir la reconversion — avant d'y engager un an de votre vie. Éligible CPF.", "tag": "Bilan"}, {"titre": "Accompagnement emploi", "desc": "CV métier, réseau d'entreprises partenaires, préparation aux entretiens : le titre est un moyen, l'emploi est le but.", "tag": "Emploi"}];
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          {fd?.logoBase64 ? (
            ...
            <>
              ...
              <span style={{ fontFamily: FONT, fontSize: 18, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "Alma Compétences"))}</span>
...
<section className="i348-hero" style={{ minHeight: "100dvh", display: "grid", gridTemplateColumns: "minmax(0,1.08fr) minmax(0,0.92fr)", gap: 56, alignItems: "center", padding: "140px 64px 70px", maxWidth: 1260, margin: "0 auto" }}>
        <div>
          ...
          <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }} style={{ fontSize: 16.5, color: C.textMuted, lineHeight: 1.75, maxWidth: 480, marginBottom: 32 }}>
            {fd?.tagline ?? c?.heroSubline ?? "Cuisine, menuiserie, développement web : des reconversions en 4 à 9 mois vers des titres professionnels RNCP, en atelier réel, avec stage et accompagnement à l'emploi."}
          </motion.p>
      ...
      <section id="engagements" className="i348-pad" style={{ padding: "96px 64px", background: C.bgSection }}>
        <div className="i348-split" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 64, alignItems: "center" }}>
          <Reveal>
            <div style={{ borderRadius: 12, border: `1px solid ${C.border}`, background: C.accentLight, aspectRatio: "4/3", justifyContent: "center" , overflow: "hidden" }}><img src={photo(0, (clientPhotos(sessionData)[0] || "https://images.pexels.com/photos/34007246/pexels-photo-34007246.jpeg?auto=compress&cs=tinysrgb&w=1400"))} alt="Travaux pratiques en atelier" loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /></div>
          </Reveal>
          <Reveal delay={0.15}>
            <div>
        ...
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              ...
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 3.5vw, 44px)", color: C.text, marginTop: 10 }}>{/* TEXTE_SECTION */ clientText(sessionData, "tarifs.titre") ?? (<>Rarement <em>de votre poche.</em></>)}</h2>
      ...
      <section className="i348-pad" style={{ padding: "96px 64px", background: C.bgDark }}>
        <Reveal>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <h2 style={{ fontFamily: FONT, fontSize: "clamp(26px, 3.4vw, 42px)", color: "#fff" }}>{/* TEXTE_SECTION */ clientText(sessionData, "section-7.titre") ?? (<>Ils ont <em style={{ color: C.hi }}>changé de métier</em>.</>)}</h2>
          </div>
        </Reveal>
      ...
      <footer className="i348-pad" style={{ background: C.bgDark, padding: "44px 64px 22px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 28, marginBottom: 30 }}>
            <div>
              <div style={{ fontFamily: FONT, fontSize: 18, color: C.hi, marginBottom: 8 }}>{fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "Alma Compétences"))}</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[{ icon: <MapPin size={13} />, t: (clientCity(sessionData) ?? "Bordeaux") + ", Gironde" }, { icon: <Phone size={13} />, t: phone }, { icon: <Clock size={13} />, t: "Lun–Ven 8h30–18h30" }].map((item, idx) => (
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.09)", paddingTop: 14, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
            <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 12 }}>
              © 2026 {fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "Alma Compétences"))} — Site réalisé par Aevia WS · SIREN {/* VILLE_PIED */}{clientCity(sessionData) ? ` · ${clientCity(sessionData)}` : ""}<LegalIdentity />
            </span>
