...
const AVIS_SOURCE = [{"texte": "Mariage de 180 personnes sous la verrière : la cérémonie à la lumière du soir, le dîner dans la nef, le DJ jusqu'à 4 h. Le régisseur a géré une panne de traiteur sans que personne ne s'en aperçoive.", "auteur": "Lisa & Karim", "detail": "Mariage urbain"}, {"texte": "Lancement produit devant 300 invités : accès camions impeccable, son calibré, keynote au cordeau. Le lieu a fait le buzz autant que le produit.", "auteur": "Dir. marketing, marque lilloise", "detail": "Lancement produit"}, {"texte": "Trois jours de tournage : électricité stable, régisseur qui connaît chaque recoin, brique magnifique à l'image. On revient au printemps.", "auteur": "Prod. audiovisuelle parisienne", "detail": "Tournage"}];
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          {fd?.logoBase64 ? (
            ...
            <>
              ...
              <span style={{ fontFamily: FONT, fontSize: 18, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "Halle 1897"))}</span>
        ...
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              ...
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 3.5vw, 44px)", color: C.text, marginTop: 10 }}>{/* TEXTE_SECTION */ clientText(sessionData, "tarifs.titre") ?? (<>Modulaires, <em>publiés.</em></>)}</h2>
      ...
      <section className="i370-pad" style={{ padding: "96px 64px", background: C.bgDark }}>
        <Reveal>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <h2 style={{ fontFamily: FONT, fontSize: "clamp(26px, 3.4vw, 42px)", color: "#fff" }}>{/* TEXTE_SECTION */ clientText(sessionData, "section-7.titre") ?? (<>La halle <em style={{ color: C.hi }}>a encore tourné</em>.</>)}</h2>
          </div>
        </Reveal>
      ...
      <footer className="i370-pad" style={{ background: C.bgDark, padding: "44px 64px 22px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 28, marginBottom: 30 }}>
            <div>
              <div style={{ fontFamily: FONT, fontSize: 18, color: C.hi, marginBottom: 8 }}>{fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "Halle 1897"))}</div>
