...
import { ArrowRight, BadgeCheck, CheckCircle, Clock, Eye, Mail, MapPin, Phone, Shield, Star } from "lucide-react";
...
/* Société de sécurité privée, 1re variante. Signature : GhostSolid — la présence qui se matérialise. Sans photographie. */
...
const NAV = [{"l": "Dispositifs", "h": "#services"}, {"l": "La méthode", "h": "#methode"}, {"l": "Tarifs", "h": "#tarifs"}, {"l": "Contact", "h": "#contact"}];
const HERO = [];

const SERVICES_SOURCE = [{"titre": "Gardiennage de sites", "desc": "Postes fixes jour/nuit : industries, chantiers, sièges. Agents cartés, consignes écrites, main courante électronique consultable.", "tag": "Sites"}, {"titre": "Rondes & interventions", "desc": "Rondes aléatoires horodatées, levée de doute sur alarme, intervention sous 15 minutes en ville — rapport photo à chaque passage.", "tag": "Rondes"}, {"titre": "Événementiel", "desc": "Concerts, salons, matchs : palpations (agents habilités), filtrage, évacuation. Dimensionnement selon les normes préfectorales.", "tag": "Événement"}, {"titre": "Sécurité incendie (SSIAP)", "desc": "Agents SSIAP 1 à 3 pour ERP et IGH : la sécurité des personnes est une qualification, pas une option.", "tag": "SSIAP"}, {"titre": "Télésurveillance", "desc": "Vidéo analysée depuis notre PC 24h/24, levée de doute humaine avant toute intervention — fini les déplacements pour un chat.", "tag": "Vidéo"}, {"titre": "Arrière-boutiques du luxe", "desc": "Boutiques, joailleries, événements privés : agents en civil, discrétion contractuelle, protocoles écrits.", "tag": "Discret"}];
...
export default function SentinellePage() {
  ...
  const phone = fd?.phone ?? "04 91 00 00 01";
  const telHref = `tel:${fd?.phone ?? "+33491000001"}`;
  const mail = fd?.email ?? "operations@sentinelle-sud.fr";

  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: FONT_BODY, overflowX: "clip" }}>
      <style>{`
        @media (max-width: 900px) { #i371-nav { display: none !important; } .i371-burger { display: flex !important; } }
        @media (max-width: 860px) {
          .i371-hero { grid-template-columns: 1fr !important; padding: 118px 24px 46px !important; gap: 34px !important; }
          .i371-card { max-width: 380px; margin: 0 auto; width: 100%; }
          .i371-split { grid-template-columns: 1fr !important; }
          .i371-stats { grid-template-columns: 1fr 1fr !important; row-gap: 8px; }
          .i371-stats .i371-statcell { border-right: none !important; }
          .i371-pad { padding-left: 24px !important; padding-right: 24px !important; }
          .i371-herotext { padding: 0 24px 44px !important; }
        }
      `}</style>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, height: 72, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 48px", background: scrolled ? C.bg : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", borderBottom: `1px solid ${scrolled ? C.border : "transparent"}`, transition: "all 0.4s ease" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          {fd?.logoBase64 ? (
            ...
            <>
              ...
              <span style={{ fontFamily: FONT, fontSize: 18, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "Sentinelle Sud"))}</span>
              <span style={{ fontSize: 10, letterSpacing: 2.2, textTransform: "uppercase", color: C.textMuted, marginLeft: 6 }}>Sécurité privée</span>
            </>
          )}
        </div>
        <div id="i371-nav" style={{ display: "flex", gap: 24, alignItems: "center" }}>
          {NAV.map(({ l, h }) => (
        ...
        <button className="i371-burger" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Menu" style={{ display: "none", flexDirection: "column", justifyContent: "center", gap: 5, background: "none", border: "none", cursor: "pointer", padding: 10, minWidth: 44, minHeight: 44 }}>
      ...
      <section className="i371-hero" style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "140px 64px 70px", maxWidth: 1080, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: 16.5, color: C.textMuted, lineHeight: 1.75, maxWidth: 560, margin: "14px 0 32px" }}>
          {clientAccrocheRestante(sessionData, 1, 18) ?? c?.heroSubline ?? "Gardiennage, rondes, événementiel, télésurveillance : des agents cartés CNAPS, encadrés, équipés'et contrôlés — parce que la sécurité est un métier réglementé, pas un gilet jaune fluo."}
        </p>
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
          <motion.a href={telHref} style={{ background: C.accent, color: "#101010", borderRadius: 8, padding: "15px 30px", fontWeight: 700, fontSize: 15, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 9 }} whileHover={{ scale: 1.02 }}>
            Évaluer votre site <ArrowRight size={16} />
          </motion.a>
      ...
      <section style={{ background: C.bgDark }}>
        <div className="i371-stats i371-pad" style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", maxWidth: 1100, margin: "0 auto", padding: "0 32px" }}>
          {STATS.map((s, idx) => (
            <Reveal key={s.label} delay={idx * 0.08}>
              <div className="i371-statcell" style={{ padding: "30px 8px", textAlign: "center", borderRight: idx < 3 ? "1px solid rgba(255,255,255,0.08)" : "none" }}>
      ...
      {/* ── SERVICES ────────────────────────────────────────────────────── */}
      <section id="services" className="i371-pad" style={{ padding: "96px 64px", background: C.bgSection }}>
      ...
      {/* ── MÉTHODE / INFOS ─────────────────────────────────────────────── */}
      <section id="methode" className="i371-pad" style={{ padding: "96px 64px", background: C.bg }}>
          ...
          <Reveal>
            <div style={{ marginBottom: 50 }}>
              ...
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 3.8vw, 46px)", color: C.text, marginTop: 10, lineHeight: 1.14 }}>{/* TEXTE_SECTION */ clientText(sessionData, "methode.titre") ?? (<>
                Des consignes écrites,<br /><em>des comptes rendus lisibles.</em>
              </>)}</h2>
            </div>
          </Reveal>
      ...
      {/* ── ENGAGEMENTS ─────────────────────────────────────────────────── */}
      <section id="engagements" className="i371-pad" style={{ padding: "96px 64px", background: C.bgSection }}>
        <div className="i371-split" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 64, alignItems: "center" }}>
          <Reveal>
            <div style={{ borderRadius: 12, border: `1px solid ${C.border}`, background: C.accentLight, aspectRatio: "4/3", justifyContent: "center" , overflow: "hidden" }}><img src={photo(0, (clientPhotos(sessionData)[0] || "https://images.pexels.com/photos/35562107/pexels-photo-35562107.png?auto=compress&cs=tinysrgb&w=1400"))} alt="Agent de sécurité en poste" loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /></div>
          </Reveal>
          <Reveal delay={0.15}>
            <div>
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", color: C.accent }}>Nos garanties</span>
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(26px, 3vw, 40px)", color: C.text, margin: "12px 0 26px", lineHeight: 1.18 }}>{/* TEXTE_SECTION */ clientText(sessionData, "engagements.titre") ?? (<>
                Réglementés,<br /><em>et contrôlables.</em>
              </>)}</h2>
      ...
      {/* ── TARIFS ──────────────────────────────────────────────────────── */}
      <section id="tarifs" className="i371-pad" style={{ padding: "96px 64px", background: C.bg }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", color: C.accent }}>Tarifs</span>
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 3.5vw, 44px)", color: C.text, marginTop: 10 }}>{/* TEXTE_SECTION */ clientText(sessionData, "tarifs.titre") ?? (<>Au juste prix <em>d'un agent déclaré.</em></>)}</h2>
              <p style={{ fontSize: 15, color: C.textMuted, maxWidth: 560, margin: "14px auto 0", lineHeight: 1.7 }}>Un tarif anormalement bas signifie toujours travail dissimulé ou agent non carté — les donneurs d'ordre sont co-responsables. Nos prix tiennent la route et les contrôles.</p>
            </div>
          </Reveal>
      ...
      {/* ── AVIS ────────────────────────────────────────────────────────── */}
      <section className="i371-pad" style={{ padding: "96px 64px", background: C.bgDark }}>
        <Reveal>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <h2 style={{ fontFamily: FONT, fontSize: "clamp(26px, 3.4vw, 42px)", color: "#fff" }}>{/* TEXTE_SECTION */ clientText(sessionData, "section-7.titre") ?? (<>Des sites <em style={{ color: C.hi }}>tenus</em>.</>)}</h2>
          </div>
        </Reveal>
      ...
      {/* ── CONTACT ─────────────────────────────────────────────────────── */}
      <section id="contact" className="i371-pad" style={{ padding: "96px 64px", background: C.accentLight, textAlign: "center" }}>
        <Reveal>
          <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", color: C.accent }}>Audit offert</span>
          <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 4vw, 48px)", color: C.text, margin: "14px 0 16px" }}>{/* TEXTE_SECTION */ clientText(sessionData, "contact.titre") ?? (<>
            Votre site a des failles.<br /><em>Autant les connaître.</em>
          </>)}</h2>
          <p style={{ fontSize: 16, color: C.textMuted, maxWidth: 460, margin: "0 auto 36px", lineHeight: 1.7 }}>Audit de sûreté offert, rapport écrit remis même si vous ne donnez pas suite. PC opérationnel joignable 24h/24.</p>
      ...
      {/* ── FOOTER ──────────────────────────────────────────────────────── */}
      <footer className="i371-pad" style={{ background: C.bgDark, padding: "44px 64px 22px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 28, marginBottom: 30 }}>
            <div>
              <div style={{ fontFamily: FONT, fontSize: 18, color: C.hi, marginBottom: 8 }}>{fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "Sentinelle Sud"))}</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[{ icon: <MapPin size={13} />, t: (clientCity(sessionData) ?? "Marseille") + ", Bouches-du-Rhône" }, { icon: <Phone size={13} />, t: phone }, { icon: <Clock size={13} />, t: "PC opérationnel 24h/24 · bureaux Lun–Ven 9h–18h" }].map((item, idx) => (
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.09)", paddingTop: 14, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
            <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 12 }}>
              © 2026 {fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "Sentinelle Sud"))} — Site réalisé par Aevia WS · SIREN {/* VILLE_PIED */}{clientCity(sessionData) ? ` · ${clientCity(sessionData)}` : ""}<LegalIdentity />
            </span>
