    ...
    <div style={{ background: C.bg, fontFamily: FONT_BODY, overflowX: "hidden" }}>
      ...
      <section id="hero" ref={heroRef} style={{ height: "100dvh", minHeight: "640px", position: "relative", display: "flex", alignItems: "flex-end", overflow: "hidden" }}>
        <motion.div style={{ y: heroY, position: "absolute", inset: 0 }}>
          <img src={photo(0, (clientPhotos(sessionData)[0] || "https://images.unsplash.com/photo-1598371839696-5c5bb00bdc28?w=1920&q=80"))} alt="Studio tatouage Encre Noire Paris" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        </motion.div>
        ...
        <motion.div className="mb226-hero-text" style={{ position: "relative", zIndex: 1, padding: "0 80px 90px", maxWidth: 780, y: heroTextY, opacity: heroOpacity }}>
          <motion.h1 initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.9 }}
            style={{ fontFamily: FONT, fontSize: "clamp(42px, 5.5vw, 74px)", fontWeight: 400, color: C.text, lineHeight: 1.05, marginBottom: 24 }}>{/* TEXTE_SECTION */ clientText(sessionData, "hero.titre") ?? (<>{<>{clientHeroLine(sessionData, 0, 2, 15) ?? "L'art sur peau,"}<br /><em style={{ color: C.accent }}>{clientHeroLine(sessionData, 1, 2, 15) ?? "pour toujours."}</em>
          </>}</>)}</motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.75 }}
            style={{ fontSize: 17, color: "rgba(245,240,232,0.65)", lineHeight: 1.75, marginBottom: 40, maxWidth: 520 }}>{c?.heroSubline ?? clientHeroSubtitle(sessionData) ?? <>
      ...
      <section style={{ padding: "100px 80px", background: C.bgSection }}>
        <div className="imx-mobstack" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
          <Reveal>
            <img src={photo(1, (clientPhotos(sessionData)[1] || "https://images.unsplash.com/photo-1562962230-16e4623d36e6?w=800&q=80"))} alt="Artiste tatoueur au travail" style={{ width: "100%", borderRadius: 8, aspectRatio: "4/3", objectFit: "cover" }} />
          </Reveal>
          <Reveal delay={0.15}>
