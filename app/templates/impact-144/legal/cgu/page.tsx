...
export default function Page() {
  return (
    <div className="bg-[#0a0506] min-h-screen text-[10px] font-sans selection:bg-white/10">
      <header className="fixed top-0 left-0 right-0 z-50 px-8 py-6 bg-black/40 backdrop-blur-md border-b border-white/10">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <Link href="/templates/impact-144" className="flex items-center gap-2 text-xs tracking-widest uppercase hover:opacity-80 transition-opacity">
            <ArrowLeft className="w-4 h-4" />
            <span>Retour à l&apos;accueil</span>
          </Link>
          <span className="text-sm font-semibold uppercase tracking-widest text-[10px]">IMPACT-144</span>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-6 pt-36 pb-24">
        ...
        <div className="space-y-8 text-sm leading-relaxed opacity-80 border-t border-white/10 pt-8">
          <section className="space-y-3">
            ...
            <p>Le site web est édité par la société exploitant le modèle IMPACT-144, société au capital social de 100 000 €, immatriculée au RCS sous le numéro 901 234 567.</p>
          </section>
      ...
      <footer className="border-t border-white/10 py-8 px-6 text-center text-xs opacity-40">
        <p>&copy; 2026 IMPACT-144. Tous droits réservés.</p>
      </footer>
    </div>
  )
}
