    ...
    <div className="bg-[#fdfaf7] text-[#1a1018] overflow-x-hidden" style={{ fontFamily: "'Didact Gothic', 'Inter', system-ui, sans-serif" }}>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${scrolled ? "bg-[#fdfaf7]/97 backdrop-blur-xl py-3 shadow-sm border-b border-[var(--brand,#c4a06a)]/12" : "bg-transparent py-7"}`}>
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0493567890").replace(/[^+0-9]/g, "")}`} className="hidden md:flex items-center gap-2 text-[var(--brand,#c4a06a)] font-bold text-sm">
              <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "04 93 56 78 90"}
            </a>
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="bg-[#fdfaf7] border-slate-100 p-10">
                <div className="flex flex-col gap-7 mt-16">
                  ...
                  <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0493567890").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 text-[var(--brand,#c4a06a)] font-bold text-xl mt-4"><Phone className="w-5 h-5" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "04 93 56 78 90"}</a>
                </div>
              </SheetContent>
            </Sheet>
          </div>
      ...
      <section id="hero" ref={heroRef} className="relative h-[100dvh] min-h-[640px] flex [align-items:safe_flex-end] overflow-hidden pt-24 sm:pt-0">
        <motion.div style={{ y: heroY }} className="absolute inset-0">
          <Image src={photo(0, (clientPhotos(sessionData)[0] || "https://images.unsplash.com/photo-1519741497674-611481863552?auto=format&fit=crop&q=88&w=2400"))} alt="Mariage élégant décoration florale" fill className="object-cover object-center" priority style={{ filter: "brightness(0.38) saturate(0.85)" }} />
        ...
        <motion.div style={{ y: heroTextY, opacity: heroOpacity }} className="relative z-10 max-w-[1400px] w-full mx-auto px-6 md:px-12 pb-32">
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.0 }} className="flex flex-wrap gap-4">
            ...
            <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0493567890").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 px-9 py-4 border border-[#fdfaf7]/12 text-[#fdfaf7]/38 font-bold text-[10px] uppercase tracking-widest hover:border-[var(--brand,#c4a06a)]/40 hover:text-[var(--brand,#c4a06a)] transition-all">
              <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "04 93 56 78 90"}
            </a>
          </motion.div>
        </motion.div>
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-10">
          <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2.6 }} className="w-[1px] h-12 bg-gradient-to-b from-[var(--brand,#c4a06a)]/40 to-transparent" />
        </div>
      </section>
      ...
      <section id="formules" className="py-28 bg-[#fdfaf7]">
        <div className="max-w-[1300px] mx-auto px-6 md:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {FORMULES.map((f, i) => (
              <Reveal key={i} delay={i * 0.07}>
                <div className="group p-8 bg-white border border-[#f0e6dc] hover:border-[var(--brand,#c4a06a)]/30 hover:shadow-xl hover:shadow-[var(--brand,#c4a06a)]/5 transition-all duration-500 h-full">
                  <div className="w-10 h-10 bg-[var(--brand,#c4a06a)]/8 flex items-center justify-center mb-5 group-hover:bg-[var(--brand,#c4a06a)] transition-colors duration-500">
                    <f.icon className="w-5 h-5 text-[var(--brand,#c4a06a)] group-hover:text-white transition-colors" />
                  </div>
                  <h3 className="font-bold text-[#1a1018] mb-3 group-hover:text-[var(--brand,#c4a06a)] transition-colors" style={{ fontFamily: "'Lora', serif" }}>{f.title}</h3>
      ...
      <section id="portfolio" className="py-20 bg-white">
        <div className="max-w-[1300px] mx-auto px-6 md:px-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 h-[65vh] min-h-[420px]">
            <div className="relative overflow-hidden"><ParallaxImg src={photo(1, (clientPhotos(sessionData)[1] || "https://images.unsplash.com/photo-1465495976277-4387d4b0b4c6?auto=format&fit=crop&q=85&w=600"))} alt="Mariage" /></div>
            <div className="row-span-2 relative overflow-hidden"><ParallaxImg src={photo(2, (clientPhotos(sessionData)[2] || "https://images.unsplash.com/photo-1511795409834-ef04bbd61622?auto=format&fit=crop&q=85&w=800"))} alt="Décoration florale" /></div>
            <div className="relative overflow-hidden"><ParallaxImg src={photo(3, (clientPhotos(sessionData)[3] || "https://images.unsplash.com/photo-1532712938310-34cb3982ef74?auto=format&fit=crop&q=85&w=600"))} alt="Couple" /></div>
            <div className="relative overflow-hidden"><ParallaxImg src={photo(4, (clientPhotos(sessionData)[4] || "https://images.unsplash.com/photo-1540575467063-178a50c2df87?auto=format&fit=crop&q=85&w=600"))} alt="Table de mariage" /></div>
            <div className="relative overflow-hidden"><ParallaxImg src={photo(5, (clientPhotos(sessionData)[5] || "https://images.unsplash.com/photo-1519225421980-715cb0215aed?auto=format&fit=crop&q=85&w=600"))} alt="Gâteau" /></div>
          </div>
        </div>
      </section>
              ...
              <Reveal key={i} delay={i * 0.1}>
                <div className="p-8 bg-white border border-[#f0e6dc] h-full flex flex-col">
                  <div className="flex gap-1 mb-5">
                    {[...Array(5)].map((_, j) => <Star key={j} className="w-4 h-4 fill-[var(--brand,#c4a06a)] text-[var(--brand,#c4a06a)]" />)}
                  </div>
                  ...
                  <div className="mt-6 pt-5 border-t border-[#f0e6dc]">
                    ...
                    <div className="text-[10px] text-[var(--brand,#c4a06a)] mt-1 flex items-center gap-1"><MapPin className="w-3 h-3" />{t.l}</div>
                  </div>
                </div>
              </Reveal>
            ))}
        ...
        <Reveal>
          <div className="max-w-xl mx-auto px-6">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              ...
              <a href={`tel:${(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "0493567890").replace(/[^+0-9]/g, "")}`} className="flex items-center gap-3 px-10 py-4 border border-[#fdfaf7]/12 text-[#fdfaf7]/35 font-bold text-[10px] uppercase tracking-widest hover:border-[var(--brand,#c4a06a)]/40 hover:text-[var(--brand,#c4a06a)] transition-all">
                <Phone className="w-4 h-4" /> {clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "04 93 56 78 90"}
              </a>
            </div>
          </div>
      ...
      <footer className="bg-[#100b0f] pt-20 pb-10 px-6 border-t border-white/5">
        <div className="max-w-[1300px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          {[
            ...
            { t: "Contact", ls: [(clientPhone(sessionData) ?? clientEmail(sessionData) ?? fd?.email ?? "04 93 56 78 90"), (fd?.email ?? "hello@maison-elise.fr"), (clientCity({ formData: fd }) ?? "Nice") + " & Riviera", "France entière & étranger", "Consultation gratuite"] },
          ].map((col, i) => (
            <div key={i}>
