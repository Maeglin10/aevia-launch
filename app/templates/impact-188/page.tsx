...
export default function CliniqueBoisVertPage() {
  ...
  return (
    <div className="bg-[#fdfaf6] text-[#2d2318] overflow-x-hidden" style={{ fontFamily: "'Source Sans 3', 'Inter', system-ui, sans-serif" }}>
      {/* ── NAVBAR ── */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${scrolled ? "bg-[#fdfaf6]/98 backdrop-blur-xl py-3 shadow-sm border-b border-[var(--brand,#3a7d44)]/10" : "bg-[#fdfaf6]/95 backdrop-blur-md py-5 border-b border-[var(--brand,#3a7d44)]/5"}`}>
        <div className="max-w-[1300px] mx-auto px-6 md:px-12 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            {fd?.logoBase64 ? (
              ...
              <>
                <div className="w-8 h-8 bg-[var(--brand,#3a7d44)] rounded-full flex items-center justify-center">
      ...
      <section id="hero" ref={heroRef} className="relative h-[100dvh] min-h-[640px] flex items-end overflow-hidden">
        ...
        <motion.div style={{ y: heroTextY, opacity: heroOpacity }} className="relative z-10 max-w-[1300px] w-full mx-auto px-6 md:px-12 pb-28">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="flex flex-wrap gap-5">
            {/* LISTE_LIBELLES */ (clientList(sessionData, "hero.liste1") ?? ["Urgences 7j/7 jusqu'à 20h", "Radio & écho en cabinet", "3 vétérinaires"]).map((b, i) => (
              <div key={i} className="flex items-center gap-2">
                <CheckCircle className="w-3.5 h-3.5 text-[var(--brand,#3a7d44)]" />
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
          <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2.2 }} className="w-[1px] h-10 bg-gradient-to-b from-[var(--brand,#3a7d44)]/60 to-transparent" />
        </div>
      </section>

      {/* ── URGENCES BAND ── */}
      <div className="py-9 bg-[var(--brand,#3a7d44)] text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {SOINS.map((s, i) => (
              <Reveal key={i} delay={i * 0.07}>
                <div className="group p-8 rounded-2xl bg-white border border-[#e8f5eb] hover:border-[var(--brand,#3a7d44)]/30 hover:shadow-lg hover:shadow-[var(--brand,#3a7d44)]/5 transition-all duration-500 h-full">
                  <div className="w-10 h-10 bg-[#e8f5eb] rounded-xl flex items-center justify-center mb-5 group-hover:bg-[var(--brand,#3a7d44)] transition-colors duration-500">
                    <s.icon className="w-5 h-5 text-[var(--brand,#3a7d44)] group-hover:text-white transition-colors" />
                  </div>
              ...
              <Reveal key={i} delay={i * 0.1}>
                <div className="p-8 rounded-2xl bg-[#fdfaf6] border border-[#e8f5eb] h-full flex flex-col">
                  <div className="flex gap-1 mb-5">
                    {[...Array(5)].map((_, j) => <Star key={j} className="w-4 h-4 fill-[var(--brand,#3a7d44)] text-[var(--brand,#3a7d44)]" />)}
                  </div>
                  ...
                  <div className="mt-6 pt-5 border-t border-[#e8f5eb]">
                    ...
                    <div className="text-[10px] text-[var(--brand,#3a7d44)] mt-1 flex items-center gap-1"><MapPin className="w-3 h-3" />{t.l}</div>
                  </div>
                </div>
              </Reveal>
            ))}
      ...
      {/* ── CTA ── */}
      <section id="contact" className="py-24 bg-[var(--brand,#3a7d44)] text-center">
        <Reveal>
