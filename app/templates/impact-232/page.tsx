...
let C: Record<string, string> = {
  ...
  text: "#121a08", textMuted: "var(--brand,#5a6b46)",
  accent: "var(--brand-light,#4a7a28)", accentDark: "#365a1e", accentLight: "#d4e8c0",
...
export default function VertNaturePage() {
  ...
  return (
    <div style={{ background: C.bg, fontFamily: FONT_BODY, overflowX: "hidden" }}>
      ...
      <style>{`
        @media (max-width: 900px) { #mb232-nav { display: none !important; } .mb232-burger { display: flex !important; } }
        @media (max-width: 640px) {
          #hero { height: 100dvh !important; min-height: 100dvh !important; }
          .mb232-hero-text { padding: 0 24px 40px !important; }
        }
      `}</style>
      ...
      <section id="hero" ref={heroRef} style={{ height: "100dvh", minHeight: "640px", position: "relative", display: "flex", alignItems: "flex-end", overflow: "hidden" }}>
        ...
        <motion.div className="mb232-hero-text" style={{ position: "relative", zIndex: 1, padding: "0 80px 90px", maxWidth: 760, y: heroTextY, opacity: heroOpacity }}>
          <motion.h1 initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.9 }}
            style={{ fontFamily: FONT, fontSize: "clamp(40px, 5vw, 68px)", color: "#fff", lineHeight: 1.1, marginBottom: 24 }}>{/* TEXTE_SECTION */ clientText(sessionData, "hero.titre") ?? (<>{<>{clientHeroLine(sessionData, 0, 2, 22) ?? "Votre jardin,"}<br /><em style={{ color: C.sand }}>{clientHeroLine(sessionData, 1, 2, 22) ?? "vivant et sans chimie."}</em>
          </>}</>)}</motion.h1>
