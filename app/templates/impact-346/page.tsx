...
const HERO = [
  { k: "Pâtes pressées", sub: "Les fromages d'alpage, cœur de la cave.", tiles: [{ icon: Mountain, t: "Beaufort d'alpage", d: "Été 2024, chalet d'Arêches — 14 mois de cave.", bg: "#f2e8d2", fg: "var(--brand, #6b5124)" }, { icon: Milk, t: "Comté 24 mois", d: "Fruité, sélectionné meule par meule dans le Jura.", bg: "#1e1a12", fg: "#f2e8d2" }, { icon: Clock3, t: "Abondance fermière", d: "AOP, la tomme des savoyards du dimanche.", bg: "#f3eee2", fg: "var(--brand, #6b5124)" }] },
  { k: "Chèvres & brebis", sub: "Le plateau qui change avec l'herbe.", tiles: [{ icon: Milk, t: "Chevrotins AOP", d: "Fermiers, affinés doux — la saison court d'avril à novembre.", bg: "#f3eee2", fg: "var(--brand, #6b5124)" }, { icon: Mountain, t: "Persillé de Tignes", d: "Rare, raide, magnifique. Pour ceux qui savent.", bg: "#f2e8d2", fg: "var(--brand, #6b5124)" }, { icon: Clock3, t: "Brebis des Bauges", d: "Tomme douce de printemps, croûte naturelle.", bg: "#1e1a12", fg: "#f2e8d2" }] },
  { k: "Crèmerie", sub: "Le lait dans tous ses états, en direct des fermes.", tiles: [{ icon: Milk, t: "Beurre baratté cru", d: "Baratte du mercredi, motte coupée au fil devant vous.", bg: "#1e1a12", fg: "#f2e8d2" }, { icon: Clock3, t: "Crème crue de Savoie", d: "Épaisse à la cuillère, la vraie, en pot consigné.", bg: "#f2e8d2", fg: "var(--brand, #6b5124)" }, { icon: Mountain, t: "Yaourts fermiers", d: "Lait entier du Granier, huit parfums, pots en verre.", bg: "#f3eee2", fg: "var(--brand, #6b5124)" }] }
];
...
<section className="i346-hero" style={{ minHeight: "100dvh", display: "grid", gridTemplateColumns: "minmax(0,1.08fr) minmax(0,0.92fr)", gap: 56, alignItems: "center", padding: "140px 64px 70px", maxWidth: 1260, margin: "0 auto" }}>
        <div>
          ...
          <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }} style={{ fontSize: 16.5, color: C.textMuted, lineHeight: 1.75, maxWidth: 480, marginBottom: 32 }}>
            {fd?.tagline ?? c?.heroSubline ?? "Beaufort d'alpage, tommes fermières, chèvres du plateau : achetés jeunes aux fermes, affinés dans notre cave voûtée, vendus à leur heure exacte. Goûtez la différence d'un affineur."}
          </motion.p>
      ...
      <section id="engagements" className="i346-pad" style={{ padding: "96px 64px", background: C.bgSection }}>
        <div className="i346-split" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 64, alignItems: "center" }}>
          <Reveal>
            <div style={{ borderRadius: 12, border: `1px solid ${C.border}`, background: C.accentLight, aspectRatio: "4/3", justifyContent: "center" , overflow: "hidden" }}><img src={photo(0, (clientPhotos(sessionData)[0] || "https://images.pexels.com/photos/16214484/pexels-photo-16214484.jpeg?auto=compress&cs=tinysrgb&w=1400"))} alt="Comptoir de la fromagerie" loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /></div>
          </Reveal>
          <Reveal delay={0.15}>
            <div>
        ...
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              ...
              <h2 style={{ fontFamily: FONT, fontSize: "clamp(28px, 3.5vw, 44px)", color: C.text, marginTop: 10 }}>{/* TEXTE_SECTION */ clientText(sessionData, "tarifs.titre") ?? (<>Composés, <em>équilibrés, étiquetés.</em></>)}</h2>
      ...
      <section className="i346-pad" style={{ padding: "96px 64px", background: C.bgDark }}>
        <Reveal>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <h2 style={{ fontFamily: FONT, fontSize: "clamp(26px, 3.4vw, 42px)", color: "#fff" }}>{/* TEXTE_SECTION */ clientText(sessionData, "section-7.titre") ?? (<>La Savoie <em style={{ color: C.hi }}>à l'heure juste</em>.</>)}</h2>
          </div>
        </Reveal>
