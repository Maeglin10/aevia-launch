      ...
      <section style={{ padding: "100px 80px", background: C.bgSection }}>
        <div className="imx-mobstack" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
          <Reveal delay={0.1}><div>
            ...
            {(clientList(sessionData, "apropos.liste1") ?? (clientName(sessionData) ? ["Produits professionnels sélectionnés", "Réservation en ligne ou sans rendez-vous", "Accueil soigné, conseils personnalisés"] : VALEURS)).map((v, i) => (
      ...
      <section id="contact" style={{ padding: "100px 80px", background: C.accentLight, textAlign: "center" }}>
        <Reveal>
          ...
          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <motion.a href={`mailto:${clientEmail(sessionData) ?? fd?.email ?? "hello@lebarberclub.fr"}`} style={{ background: "transparent", color: C.text, border: `2px solid ${C.accent}`, borderRadius: 6, padding: "13px 32px", fontWeight: 600, fontSize: 15, textDecoration: "none", display: "flex", alignItems: "center", gap: 8, minHeight: 44 }} whileHover={{ background: C.accent, color: C.white }}>
              <Mail size={17} /> Écrire
            </motion.a>
          </div>
        </Reveal>
