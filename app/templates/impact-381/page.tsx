...
export default function CaveDesTerroirsPage() {
  ...
  const phone = fd?.phone ?? "02 47 00 00 02";
  const telHref = `tel:${fd?.phone ?? "+33247000002"}`;
  const mail = fd?.email ?? "cave@cave-des-terroirs.fr";
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
          {fd?.logoBase64 ? (
            ...
            <>
              ...
              <span style={{ fontFamily: FONT, fontSize: 18, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "La Cave des Terroirs"))}</span>
...
<section className="i381-hero" style={{ minHeight: "100dvh", display: "grid", gridTemplateColumns: "minmax(0,1.08fr) minmax(0,0.92fr)", gap: 56, alignItems: "center", padding: "140px 64px 70px", maxWidth: 1260, margin: "0 auto" }}>
        <div>
          ...
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.85, ease: [0.16, 1, 0.3, 1] }} style={{ fontFamily: FONT, fontSize: "clamp(34px, 4.6vw, 60px)", color: C.text, lineHeight: 1.1, margin: "18px 0 20px" }}>
            {c?.heroHeadline ?? (<>Des vins qu'on a bus<br /><em style={{ color: C.accent }}>avant de vous les vendre.</em></>)}
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }} style={{ fontSize: 16.5, color: C.textMuted, lineHeight: 1.75, maxWidth: 480, marginBottom: 32 }}>
            {fd?.tagline ?? c?.heroSubline ?? "Quatre cents références choisies domaine par domaine, dont soixante en Loire : un caviste qui a serré la main de la plupart de ses vignerons et qui vous dira franchement quand une bouteille n'est pas pour vous."}
          </motion.p>
        ...
        <div className="i381-card">
          <HeldSwap index={i} tilt={9}>
            <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden", boxShadow: "0 18px 52px rgba(0,0,0,0.18)" }}>
              <div style={{ aspectRatio: "4/3", background: C.accentLight , overflow: "hidden" }}><img src={photo(0, "https://images.pexels.com/photos/35474950/pexels-photo-35474950.jpeg?auto=compress&cs=tinysrgb&w=1400")} alt="Cave du caviste" loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /></div>
      ...
      <section className="i381-pad" style={{ padding: "96px 64px", background: C.bgDark }}>
        <Reveal>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <h2 style={{ fontFamily: FONT, fontSize: "clamp(26px, 3.4vw, 42px)", color: "#fff" }}>Des bouteilles <em style={{ color: C.hi }}>bien conseillées</em>.</h2>
          </div>
        </Reveal>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.09)", paddingTop: 14, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
            <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 12 }}>
              © 2026 {fd?.businessName ?? (clientName(sessionData) ?? (clientName(sessionData) ?? "La Cave des Terroirs"))} — Site réalisé par Aevia WS · SIREN {/* VILLE_PIED */}{clientCity(sessionData) ? ` · ${clientCity(sessionData)}` : ""}<LegalIdentity />
            </span>
