         ...
         <div className="hidden md:flex gap-10 text-[10px] font-black uppercase tracking-widest text-white/40">
            {[
              ...
              <Link 
                key={item.key} 
                href={item.path} 
                className={`hover:text-white transition-colors ${item.key === "fleet" ? "text-blue-500 font-extrabold" : ""}`}
              >
                [ {item.name.replace(" ", "_")} ]
              </Link>
            ))}
                  ...
                  <div className="flex flex-col gap-8 mt-16 text-left font-mono">
                     {[
                       ...
                       <Link 
                         key={item.key} 
                         href={item.path} 
                         className={`text-xl font-black uppercase tracking-widest hover:text-blue-500 transition-colors ${item.key === "fleet" ? "text-blue-500" : "text-white/60"}`}
                       >
                         {item.name.replace(" ", "_")}
                       </Link>
                     ))}
              ...
              {FLEET.map((vehicle, idx) => (
                <div key={vehicle.id} className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center border-t border-white/5 pt-16">
                  ...
                  <div className="lg:col-span-5">
                    ...
                    <Link href="/templates/impact-08/atelier" className="w-full py-5 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-colors italic inline-block text-center">
