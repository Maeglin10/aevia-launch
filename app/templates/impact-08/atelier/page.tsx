         ...
         <div className="hidden md:flex gap-10 text-[10px] font-black uppercase tracking-widest text-white/40">
            {[
              ...
              <Link 
                key={item.key} 
                href={item.path} 
                className={`hover:text-white transition-colors ${item.key === "atelier" ? "text-blue-500 font-extrabold" : ""}`}
              >
                [ {item.name.replace(" ", "_")} ]
              </Link>
            ))}
                  ...
                  <div className="flex flex-col gap-8 mt-16 text-left font-mono">
                     {[
                       ...
                       <Link 
                         key={item.key} 
                         href={item.path} 
                         className={`text-xl font-black uppercase tracking-widest hover:text-blue-500 transition-colors ${item.key === "atelier" ? "text-blue-500" : "text-white/60"}`}
                       >
                         {item.name.replace(" ", "_")}
                       </Link>
                     ))}
                  ...
                  {stages.map((stage, i) => (
                    <div key={i} className="relative">
                      <div className="absolute -left-[41px] top-1 w-5 h-5 rounded-full bg-black border border-blue-500 flex items-center justify-center">
                         <span className="text-[10px] font-mono text-blue-500">{i+1}</span>
                      </div>
