...
let C: Record<string, string> = {
  ...
  bgDarkAlt: '#100d04',
  bgCard: '#ffffff',
  accent: 'var(--brand,#a06828)',
  accentDark: 'var(--brand-light,#7a5020)',
  accentLight: '#f0ddb8',
  white: '#ffffff',
  ink: '#1a1408',
  textMuted: '#4a3818',
...
function MenuCards() {
  const MENU_ITEMS = resolveList(
    bp?.menu?.map((m: any, i: number) => ({
      ...
      category: m.category ?? MENU_ITEMS_DEMO[i % MENU_ITEMS_DEMO.length].category,
      emoji: MENU_ITEMS_DEMO[i % MENU_ITEMS_DEMO.length].emoji,
    })),
    MENU_ITEMS_DEMO
  );
  const sec: React.CSSProperties = {
