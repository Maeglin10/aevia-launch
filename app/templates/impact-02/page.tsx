...
function PROJECTS_DEMO_LIVE() {
  ...
  { id: 4, title: "Skin & Silk", category: "Fashion", year: "2024", image: (clientPhotos(sessionData)[3] || "https://images.unsplash.com/photo-1509631179647-0177331693ae?q=80&w=1200&auto=format&fit=crop"), color: "#d97706", desc: "Editorial series for Maison Lumière's haute couture collection. Merging classical portraiture with contemporary fashion storytelling.", role: "Fashion Photographer", client: "Maison Lumière", duration: "2 mois" },
  { id: 5, title: "Volcanic Silence", category: "Landscape", year: "2024", image: (clientPhotos(sessionData)[4] || "https://images.pexels.com/photos/7172650/pexels-photo-7172650.jpeg?auto=compress&cs=tinysrgb&w=1600"), color: "#8b5cf6", desc: "The aftermath of eruption — where destruction creates a canvas more beautiful than any human could conceive. Shot over three expeditions.", role: "Documentary Photographer", client: "BBC Earth", duration: "8 mois" },
  { id: 6, title: "Human Form", category: "Portrait", year: "2026", image: (clientPhotos(sessionData)[5] || "https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?q=80&w=1200&auto=format&fit=crop"), color: "#ec4899", desc: "An intimate portrait series celebrating the raw beauty of the human form — unretouched, unfiltered, unapologetically real.", role: "Portrait Photographer", client: "Vogue Italia", duration: "5 semaines" },
  { id: 7, title: "Desert Shadows", category: "Landscape", year: "2025", image: (clientPhotos(sessionData)[6] || "https://images.unsplash.com/photo-1473580044384-7ba9967e16a0?q=80&w=1200&auto=format&fit=crop"), color: "#f59e0b", desc: "A minimalist approach to the Sahara. Using harsh midday light to create striking geometric compositions out of shifting dunes.", role: "Photographer", client: "Condé Nast Traveler", duration: "4 semaines" },
  { id: 8, title: "Urban Pulse", category: "Street", year: "2023", image: (clientPhotos(sessionData)[7] || "https://images.unsplash.com/photo-1519681393784-d120267933ba?q=80&w=1200&auto=format&fit=crop"), color: "#3b82f6", desc: "The chaos of New York City distilled into frozen moments. A monochrome series focusing on the isolation within the crowd.", role: "Street Photographer", client: "The New Yorker", duration: "3 mois" },
]);
}
...
function SERVICES_SOURCE_LIVE() {
  return [
  {
    ...
    bgImage: (clientPhotos(sessionData)[8] || "https://images.pexels.com/photos/38873706/pexels-photo-38873706.jpeg?auto=compress&cs=tinysrgb&w=1600"),
  },
  {
    id: "02",
    title: "Art Direction",
...
export default function CreativePortfolioSPA() {
  ...
  useEffect(() => {
    ...
    let n = 3;
    const _photoArrays: any[] = [PROJECTS, SERVICES];
    ...
    <div className="premium-theme bg-[#0a0a0a] text-white min-h-dvh selection:bg-amber-400 selection:text-black overflow-x-hidden font-sans">
      ...
      <section id="hero" className="relative h-dvh w-full flex items-center justify-center overflow-hidden">
        <motion.div style={{ y: heroY, scale: scaleProgress }} className="absolute inset-0 z-0 origin-bottom">
          <Image src={photo(12, "https://images.unsplash.com/photo-1509631179647-0177331693ae?q=80&w=1600&auto=format&fit=crop")} fill className="object-cover" alt="Hero" priority />
        ...
        <motion.div style={{ opacity: heroOpacity, y: heroTextY }} className="relative z-10 text-center px-6 mt-20">
          ...
          <div className="overflow-hidden mb-2">
            <motion.h1 initial={{ y: "100%" }} animate={{ y: 0 }} transition={{ duration: 1.2, ease: [0.25, 0.1, 0.25, 1], delay: 0.7 }} className="text-7xl sm:text-8xl md:text-[10rem] lg:text-[12rem] font-extralight tracking-[-0.04em] leading-[0.85]">{c?.heroHeadline ?? <>{clientHeroLine(sessionData, 0, 2, 12) ?? (<>
              Elena
            </>)}</>}</motion.h1>

          </div>
          <div className="overflow-hidden mb-12">
            <motion.h1 initial={{ y: "100%" }} animate={{ y: 0 }} transition={{ duration: 1.2, ease: [0.25, 0.1, 0.25, 1], delay: 0.85 }} className="text-7xl sm:text-8xl md:text-[10rem] lg:text-[12rem] font-black tracking-[-0.04em] leading-[0.85] italic">{clientHeroLine(sessionData, 1, 2, 12) ?? (<>
              Korr<span className="text-amber-400">.</span>
            </>)}</motion.h1>
          </div>

          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }} className="text-base md:text-xl text-white/50 max-w-2xl mx-auto leading-relaxed font-light">{clientAccrocheRestante(sessionData, 2, 5) ?? c?.heroSubline ?? <>
        ...
        <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-center">
          ...
          <Reveal delay={0.2} className="lg:col-span-7">
            ...
            <p className="text-white/60 text-xl leading-relaxed mb-8 font-light">
              {clientName(sessionData) ? <>Based in {clientCity(sessionData) ?? "Paris"}, I specialize in capturing moments that transcend the ordinary — the images I am most proud of are the ones that make strangers feel something they cannot quite name.</> : <>Based between Paris and Tokyo, I specialize in capturing moments that transcend the ordinary. My work has been featured in National Geographic, Vogue, and BBC Earth — but the images I am most proud of are the ones that make strangers feel something they cannot quite name.</>}
            </p>
